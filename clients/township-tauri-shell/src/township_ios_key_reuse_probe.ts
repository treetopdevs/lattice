...
export const TOWNSHIP_IOS_KEY_REUSE_PROBE_LOG_PREFIX = "township-ios-key-reuse-probe";
export const TOWNSHIP_IOS_KEY_REUSE_PROBE_ENV = "VITE_TOWNSHIP_IOS_KEY_REUSE_PROBE";
...
export interface TownshipIosKeyReuseProbeOptions
  extends Pick<TownshipNativeWorkflowOptions, "invoke"> {
  slot?: TownshipIosKeyReuseProbeSlot;
}

export function townshipIosKeyReuseProbeEnabled(env: TownshipIosKeyReuseProbeEnv): boolean {
...
export async function logTownshipIosKeyReuseProbeFromEnv(
  ...
  env: TownshipIosKeyReuseProbeEnv =
    ((import.meta as ImportMeta & { env?: TownshipIosKeyReuseProbeEnv }).env ?? {}),
  options: TownshipIosKeyReuseProbeOptions = {},
): Promise<boolean> {
  if (!townshipIosKeyReuseProbeEnabled(env)) return false;
  await logTownshipProbeEvent(townshipIosKeyReuseProbeLogLine(status, options.slot), {
...
export function townshipIosKeyReuseProbeLogLine(
  ...
  if (status.ready) {
    ...
    fields.push(`signature_bytes=${Math.max(0, Math.round(status.signatureBytes))}`);
  } else {
    fields.push(`error=${probeToken(status.error)}`);
  }
