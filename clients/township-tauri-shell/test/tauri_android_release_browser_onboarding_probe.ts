...
try {
  ...
  await runReleaseBrowserOnboardingConvergenceProof(serial);
} finally {
  peer?.kill();
  if (serial) {
...
console.log("\x1b[32m✓ Township Android release browser onboarding convergence smoke passed\x1b[0m");
process.exit(0);

async function runReleaseBrowserOnboardingConvergenceProof(serial: string): Promise<void> {
  ...
  try {
    ...
    console.log(`  observed browser-delivered onboarding pairing save ${pairingLine.trim()}`);
  } finally {
  ...
  console.log(`  observed browser-onboarding pairing-derived sync ${pairingSyncLine.trim()}`);
  ...
  console.log(`  observed browser-onboarding pull ${pullLine.trim()}`);
  ...
  console.log(`  observed browser-onboarding author ${authorLine.trim()}`);
  ...
  console.log(`  observed browser-onboarding pending reload ${pendingReloadLine.trim()}`);
  ...
  console.log(`  observed browser-onboarding push ${pushLine.trim()}`);
  ...
  console.log(`  observed browser-onboarding peer report ${peerLine.trim()}`);
  ...
  console.log(`  observed browser-onboarding complete ${completeLine.trim()}`);
  ...
  console.log(`  observed browser-onboarding paired reload ${pairedReloadLine.trim()}`);
  ...
  console.log(`  observed browser-onboarding relaunch sync ${relaunchSyncLine.trim()}`);
...
function releaseOnboardingProbeConfigFromBuildScript(): ReleaseOnboardingProbeBuildConfig {
  ...
  assert.doesNotMatch(
    ...
    "browser onboarding probe must receive peer config from the browser-delivered pairing handoff",
  );
...
async function openBrowserPairingPageAndTap(
  ...
  for (let pageAttempt = 0; pageAttempt < 3; pageAttempt += 1) {
    ...
    await settleBrowserFirstRun(serial);
    let observed: BrowserDeliveryEvidence;
    try {
...
async function installReleaseApk(serial: string, apkPath: string): Promise<void> {
  ...
  assert.notEqual(apkPath, defaultDebugApkPath(), "release browser onboarding smoke must not install the debug APK");
