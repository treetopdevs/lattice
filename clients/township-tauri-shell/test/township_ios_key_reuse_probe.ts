...
const unavailableStatus: TownshipNativeStatus = {
  ...
  storageNamespace: "township:zoning-variance-24",
  error:
    "NSOSStatusErrorDomain Code=-34018: Client has neither application-identifier nor keychain-access-groups",
};

assert.equal(townshipIosKeyReuseProbeEnabled({}), false);
assert.equal(
  townshipIosKeyReuseProbeEnabled({
    VITE_TOWNSHIP_IOS_KEY_REUSE_PROBE: "true",
  }),
  false,
  "the probe should require an explicit build-only value",
);
assert.equal(
  townshipIosKeyReuseProbeEnabled({ VITE_TOWNSHIP_IOS_KEY_REUSE_PROBE: " 1 " }),
  true,
);
...
assert.match(
  readyLine,
  new RegExp(`^${TOWNSHIP_IOS_KEY_REUSE_PROBE_LOG_PREFIX} `),
);
...
assert.doesNotMatch(
  readyLine,
  /storageNamespace|storageEcho|native invoke ready/,
);
...
assert.match(unavailableLine, /error=unavailable/);
assert.doesNotMatch(
  unavailableLine,
  /NSOSStatusErrorDomain|34018|keychain-access-groups/,
);
...
assert.equal(
  await logTownshipIosKeyReuseProbeFromEnv(
    ...
    {
      invoke(command, args) {
        assert.equal(command, "lattice_log_ios_key_reuse_probe");
...
console.log(
  "\x1b[32m✓ iOS protected-key relaunch probe contract checks passed\x1b[0m",
);
