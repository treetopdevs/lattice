...
{:ok, [op]} = Wire.decode_ops([Map.fetch!(oracle, "expectedPeerAdmit")])
...
if Base.encode64(peer_identity.pub) != oracle["peerPubkey"] do
  raise "roster-action peer identity does not match the fixture oracle"
end
...
if accepted_id != op.id do
  raise "stable relay accepted the wrong roster-action peer operation"
end

:ok = WebSocket.close(connection)
IO.puts("ROSTER_PEER_RELAY_READY")
