...
impl ProductDatabase {
    ...
    /// Persist a replayable value only if it still has the expected contents.
    pub fn kv_compare_and_set(
        ...
        tx.execute("INSERT INTO kv (key, value) VALUES (?1, ?2) ON CONFLICT(key) DO UPDATE SET value = excluded.value", [key, next])
            .map_err(classify_sqlite_error)?;
        tx.commit().map_err(classify_sqlite_error)?;
        Ok(true)
    }
