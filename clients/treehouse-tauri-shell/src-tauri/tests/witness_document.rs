...
fn request(
    adapter: &mut WitnessDocumentSession<impl FnMut() -> Bytes32>,
    webview: &Webview<tauri::test::MockRuntime>,
    url: &Url,
) {
    adapter.navigation_requested(webview, url).unwrap();
}
...
fn first_actual_started_and_matching_finished_establish_one_session() {
    ...
    request(&mut adapter, &webview, &url);
...
fn foreign_owner_and_origin_poison_first_start() {
    ...
    assert_eq!(
        foreign.navigation_requested(other.as_ref(), &local),
        Err(SESSION_REFUSED)
    );
    assert_eq!(
    ...
    assert_eq!(
        origin.navigation_requested(&webview, &remote),
        Err(SESSION_REFUSED)
    );
    assert_eq!(
    ...
    request(&mut mismatched, &webview, &local);
    assert_eq!(
...
#[test]
fn navigation_request_immediately_invalidates_an_established_snapshot() {
    let (_app, webview) = view();
    ...
    adapter
        .page_load(&webview, PageLoadEvent::Finished, &url)
        .unwrap();
    assert!(adapter.snapshot(&webview).is_ok());
    assert_eq!(
        adapter.navigation_requested(&webview, &url),
        Err(SESSION_REFUSED)
    );
    assert!(adapter.snapshot(&webview).is_err());
    assert_eq!(
...
#[test]
fn repeated_request_before_first_start_latches_refusal() {
    let (_app, webview) = view();
    let url: Url = LOCAL.parse().unwrap();
    let mut adapter = WitnessDocumentSession::new(nonce(1), || nonce(2));
    request(&mut adapter, &webview, &url);
...
#[test]
fn request_while_first_load_is_pending_refuses_late_finished() {
    let (_app, webview) = view();
    let url: Url = LOCAL.parse().unwrap();
    let mut adapter = WitnessDocumentSession::new(nonce(1), || nonce(2));
    request(&mut adapter, &webview, &url);
