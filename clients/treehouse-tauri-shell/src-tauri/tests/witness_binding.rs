...
fn public_claims_match_beam_bytes_and_signatures_without_custody() {
    for vector in vectors() {
        ...
        let expected = STANDARD
            .decode(vector["canonicalBase64"].as_str().unwrap())
            .unwrap();
        assert_eq!(claim.canonical_bytes(), expected);
        let key: [u8; 32] = STANDARD
            .decode(fields["actualWitnessPublicKey"].as_str().unwrap())
            .unwrap()
            .try_into()
            .unwrap();
        ...
        let signature = Signature::from_slice(
            &STANDARD
                .decode(vector["signatureBase64"].as_str().unwrap())
                .unwrap(),
        )
        .unwrap();
        ...
        for name in fields.as_object().unwrap().keys() {
            ...
            changed[name] = if name == "replica" {
                "other".into()
            } else {
                STANDARD.encode([42; 32]).into()
            };
            let bytes = BindingClaim::from_public_json(&changed.to_string())
                .unwrap()
                .canonical_bytes();
...
fn malformed_public_fields_refuse_before_encoding() {
    ...
    for replica in ["".to_string(), "a".repeat(513), "🌲".repeat(129)] {
        ...
        fields["replica"] = replica.into();
        assert!(BindingClaim::from_public_json(&fields.to_string()).is_err());
        assert!(serde_json::from_value::<BindingClaim>(fields).is_err());
    }
    ...
    for extra in ["product", "alias", "purpose", "bytes"] {
        let mut fields = original.clone();
        fields[extra] = "township".into();
        assert!(BindingClaim::from_public_json(&fields.to_string()).is_err());
    }
    let duplicate = original
        .to_string()
        .replacen('{', "{\"replica\":\"other\",", 1);
    assert!(BindingClaim::from_public_json(&duplicate).is_err());
    let oversized = format!("{}{}", " ".repeat(128 * 1024), original);
    assert!(BindingClaim::from_public_json(&oversized).is_err());
}
