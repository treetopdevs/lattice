 ...
 */
export async function deriveTreehouseCatalogCutoff(input) {
    try {
        const snapshot = structuredClone(input);
        if (!text(snapshot.replica, true) || !Array.isArray(snapshot.frames) || !Array.isArray(snapshot.rejected))
            return invalid;
        const acceptedIds = new Set(), rejectedIds = new Set();
        for (const frame of snapshot.frames) {
            if (!header(frame, snapshot.replica) || acceptedIds.has(frame.id))
                return invalid;
            acceptedIds.add(frame.id);
        }
        for (const entry of snapshot.rejected) {
            if (!closed(entry, ["frame", "reason"]) || entry.reason !== "bad_signature" ||
                !object(entry.frame) || !text(entry.frame.id) || entry.frame.replica !== snapshot.replica || rejectedIds.has(entry.frame.id))
                return invalid;
            rejectedIds.add(entry.frame.id);
        }
        if (snapshot.frames.some((frame) => frame.deps.some((dep) => !acceptedIds.has(dep))))
            return invalid;
        const ops = [], rejected = [];
        let outsideGrammar = false;
        for (const [frames, accepted] of [[snapshot.frames, true], [snapshot.rejected.map((entry) => entry.frame), false]]) {
            for (const raw of frames) {
                ...
                if (!checked.portable)
                    outsideGrammar = true;
                if (accepted)
                    ops.push(checked.record);
                else
                    rejected.push({ ...checked.record, reason: "bad_signature" });
            }
        }
        if (outsideGrammar)
            return unsupported;
        ...
        const referenced = new Set(snapshot.frames.flatMap((frame) => frame.deps));
...
async function inspectFrame(raw, replica, accepted) {
    if (!header(raw, replica))
        return accepted ? "invalid" : "unsupported";
    ...
    let portable;
    try {
        const body = term(raw.body, 64), cap = term(raw.cap, 64);
        portable = body && cap;
    }
    catch (error) {
        return error instanceof UnsupportedCutoff ? "unsupported" : accepted ? "invalid" : "unsupported";
    }
    let bytes;
    try {
        bytes = canonicalBytesForCarrierOp(raw);
    }
    catch {
        return accepted ? "invalid" : "unsupported";
    }
    ...
    try {
        signatureMatches = sig.length === 64 && ed25519.verify(sig, bytes, author, { zip215: false });
    }
    catch { /* invalid signature is retained evidence */ }
    if ((hashMatches && signatureMatches) !== accepted)
        return "invalid";
...
function closed(value, fields, optional = []) {
...
function term(value, depth) {
    if (!Array.isArray(value) || typeof value[0] !== "string")
        throw new Error("malformed term");
    const [tag, item] = value;
    if (tag === "nil" && value.length === 1)
        return true;
    if (value.length !== 2)
        throw new Error("malformed term");
    if (tag === "bool" && typeof item === "boolean")
        return true;
    if (tag === "int") {
        if (typeof item === "number" && Number.isSafeInteger(item) && item >= 0)
            return true;
        if (typeof item === "string" && /^(0|[1-9][0-9]*)$/.test(item) && BigInt(item) <= maxUint64)
            return true;
        throw new Error("malformed integer");
    }
    if (tag === "bin" && canonicalBase64Bytes(item) !== null)
        return true;
    if (tag === "atom" && text(item))
        return atoms.has(item);
    if (tag === "delegation")
        return delegation(item);
    if (!["list", "tuple", "mapset", "map"].includes(tag) || !Array.isArray(item))
        throw new Error("malformed term");
    if (depth <= 0)
        throw new UnsupportedCutoff();
    let portable = true;
    for (const entry of item) {
        if (tag === "map") {
            ...
            portable = term(entry, depth - 1) && portable;
    }
    return portable;
}
function delegation(value) {
    if (!closed(value, delegationFields, ["expires_epoch"]) || !text(value.id) || !text(value.replica) ||
        ...
        !Array.isArray(value.ops) || !value.ops.every((op) => text(op)) || !Array.isArray(value.roles) || !value.roles.every((role) => text(role)))
        throw new UnsupportedCutoff();
    if (canonicalBase64Bytes(value.issuer, 32) === null || canonicalBase64Bytes(value.audience, 32) === null || canonicalBase64Bytes(value.sig, 64) === null ||
        (Object.hasOwn(value, "expires_epoch") && !(typeof value.expires_epoch === "number" && Number.isSafeInteger(value.expires_epoch) && value.expires_epoch >= 0)))
        throw new UnsupportedCutoff();
...
function recordTerm(op, rejected = false) {
    const pairs = [
        [["atom", "id"], binary(op.id)], [["atom", "bytes"], binary(op.bytes)], [["atom", "sig"], binary(op.sig)],
    ];
    if (rejected)
        pairs.push([["atom", "reason"], ["atom", "bad_signature"]]);
    return ["map", pairs];
}
