...
const textValue = {
    ...
    decode(value) {
        ...
        if (bytes === null)
            return undefined;
        try {
            return this.normalize(textDecoder.decode(bytes));
        }
        catch {
            return undefined;
        }
...
const versionValue = constrained(integerValue, (value) => value === 1);
...
const bootstrapCodec = recordCodec([
...
const entriesCodec = constrained(listCodec(entryCodec), (entries) => entries.length >= 1 && entries.length <= 13 && strictlySorted(entries.map((entry) => entry.replica)) &&
...
const catalogCodec = recordCodec([
...
const envelopeCodec = recordCodec([["catalog", "catalog", catalogCodec], ["signature", "signature", signatureValue]]);
const cutoffCodec = recordCodec([
...
const rotationCodec = recordCodec([
    ["version", "version", versionValue], ["product", "product", productValue], ["space", "space", textValue],
...
const rotationEnvelopeCodec = recordCodec([
    ["rotation", "rotation", rotationCodec], ["oldSignature", "old_signature", signatureValue], ["newSignature", "new_signature", signatureValue],
]);
export function normalizeCatalogBootstrap(value) { return bootstrapCodec.normalize(value) ?? null; }
export function catalogBootstrapToCarrierTerm(value) { return toTerm(bootstrapCodec, value); }
export function catalogBootstrapFromCarrierTerm(value) { return bootstrapCodec.decode(value) ?? null; }
/** Flat decoded command argument only; the existing codec owns closed fields and types. */
export function catalogBootstrapFromDecodedTerm(value) {
    try {
        if (value === null || typeof value !== "object" || value.type !== "map" || !Array.isArray(value.pairs))
            return null;
        const pairs = [];
        for (const pair of value.pairs) {
            if (!Array.isArray(pair) || pair.length !== 2)
                return null;
            const [key, item] = pair;
            if (key === null || typeof key !== "object" || key.type !== "atom")
                return null;
            let scalar;
            if (typeof item === "number")
                scalar = ["int", item];
            else if (item !== null && typeof item === "object" && item.type === "atom")
                scalar = ["atom", item.value];
            else if (item !== null && typeof item === "object" && item.type === "bin" && item.bytes instanceof Uint8Array)
                scalar = ["bin", base64(item.bytes)];
            else
                return null;
...
export function normalizeCatalogEntry(value) { return entryCodec.normalize(value) ?? null; }
export function catalogEntryToCarrierTerm(value) { return toTerm(entryCodec, value); }
export function catalogEntryFromCarrierTerm(value) { return entryCodec.decode(value) ?? null; }
export function normalizeTransportCatalog(value) { return catalogCodec.normalize(value) ?? null; }
export function transportCatalogToCarrierTerm(value) { return toTerm(catalogCodec, value); }
export function transportCatalogFromCarrierTerm(value) { return catalogCodec.decode(value) ?? null; }
export function normalizeCatalogEnvelope(value) { return envelopeCodec.normalize(value) ?? null; }
export function catalogEnvelopeToCarrierTerm(value) { return toTerm(envelopeCodec, value); }
export function catalogEnvelopeFromCarrierTerm(value) { return envelopeCodec.decode(value) ?? null; }
export function normalizeCatalogCutoff(value) { return cutoffCodec.normalize(value) ?? null; }
export function catalogCutoffToCarrierTerm(value) { return toTerm(cutoffCodec, value); }
export function catalogCutoffFromCarrierTerm(value) { return cutoffCodec.decode(value) ?? null; }
export function normalizeCatalogRotation(value) { return rotationCodec.normalize(value) ?? null; }
export function catalogRotationToCarrierTerm(value) { return toTerm(rotationCodec, value); }
export function catalogRotationFromCarrierTerm(value) { return rotationCodec.decode(value) ?? null; }
export function normalizeCatalogRotationEnvelope(value) { return rotationEnvelopeCodec.normalize(value) ?? null; }
export function catalogRotationEnvelopeToCarrierTerm(value) { return toTerm(rotationEnvelopeCodec, value); }
export function catalogRotationEnvelopeFromCarrierTerm(value) { return rotationEnvelopeCodec.decode(value) ?? null; }
export function canonicalBytesForTransportCatalog(value) { return domainBytes(catalogDomain, requireTerm(catalogCodec, value)); }
export function transportCatalogId(value) { return digest(canonicalBytesForTransportCatalog(value)); }
export function canonicalBytesForCatalogRotation(value) { return domainBytes(rotationDomain, requireTerm(rotationCodec, value)); }
export function canonicalBytesForCatalogRotationPossession(value) {
...
export function catalogRotationId(value) { return digest(domainBytes(rotationDomain, requireTerm(rotationEnvelopeCodec, value))); }
export function canonicalBytesForCatalogInventory(value) { return domainBytes(inventoryDomain, requireTerm(entriesCodec, value)); }
export function catalogInventoryId(value) { return digest(canonicalBytesForCatalogInventory(value)); }
export function catalogServiceRealm(value) {
    const serviceId = idValue.normalize(value);
    if (serviceId === undefined)
        throw new TypeError("malformed Treehouse service ID");
...
export function verifyCatalogEnvelope(value, expectedCatalogKey) {
    try {
        const envelope = normalizeCatalogEnvelope(value);
        const key = canonicalBase64Bytes(expectedCatalogKey, 32);
        return envelope !== null && key !== null && ed25519.verify(canonicalBase64Bytes(envelope.signature, 64), canonicalBytesForTransportCatalog(envelope.catalog), key, { zip215: false });
    }
    catch {
        return false;
    }
...
export function verifyCatalogRotationEnvelope(value, expectedOldKey) {
    try {
        const envelope = normalizeCatalogRotationEnvelope(value);
        const oldKey = canonicalBase64Bytes(expectedOldKey, 32);
        if (envelope === null || oldKey === null || expectedOldKey === envelope.rotation.newCatalogKey)
            return false;
        return ed25519.verify(canonicalBase64Bytes(envelope.oldSignature, 64), canonicalBytesForCatalogRotation(envelope.rotation), oldKey, { zip215: false }) &&
            ed25519.verify(canonicalBase64Bytes(envelope.newSignature, 64), canonicalBytesForCatalogRotationPossession(envelope.rotation), canonicalBase64Bytes(envelope.rotation.newCatalogKey, 32), { zip215: false });
    }
    catch {
        return false;
    }
}
function recordCodec(fields, valid = () => true) {
    return {
        normalize(value) {
            if (!closedRecord(value, fields.map(([key]) => key)))
                return undefined;
            const result = {};
        ...
        encode(value) { return ["map", fields.map(([key, wireKey, codec]) => [["atom", wireKey], codec.encode(value[key])])]; },
        decode(value) {
            if (!tagged(value, "map") || !Array.isArray(value[1]) || value[1].length !== fields.length)
                return undefined;
            const wire = new Map();
            for (const pair of value[1]) {
                if (!Array.isArray(pair) || pair.length !== 2 || !tagged(pair[0], "atom") || typeof pair[0][1] !== "string" ||
                    wire.has(pair[0][1]) || !fields.some(([, key]) => key === pair[0][1]))
                    return undefined;
...
function constrained(codec, valid) {
    const check = (value) => value !== undefined && valid(value) ? value : undefined;
...
function listCodec(codec) {
    function read(value, decode) {
...
function atomValue(...allowed) {
    return { normalize(value) { return typeof value === "string" && allowed.includes(value) ? value : undefined; },
        encode(value) { return ["atom", value]; }, decode(value) { return tagged(value, "atom") ? this.normalize(value[1]) : undefined; } };
}
function rawBinary(length) {
    return { normalize(value) { return canonicalBase64Bytes(value, length) === null ? undefined : value; },
        encode(value) { return ["bin", value]; }, decode(value) { return tagged(value, "bin") ? this.normalize(value[1]) : undefined; } };
}
function toTerm(codec, value) {
    const normalized = codec.normalize(value);
    return normalized === undefined ? null : codec.encode(normalized);
}
function requireTerm(codec, value) {
    const term = toTerm(codec, value);
    if (term === null)
        throw new TypeError("malformed Treehouse catalog record");
    return term;
}
function tagged(value, tag) { return Array.isArray(value) && value.length === 2 && value[0] === tag; }
function binaryBytes(value) { return tagged(value, "bin") ? canonicalBase64Bytes(value[1]) : null; }
function textTerm(value) { return ["bin", base64(textEncoder.encode(value))]; }
function domainBytes(domain, term) { return canonicalBytesForCarrierTerm(["list", [textTerm(domain), term]]); }
function digest(bytes) { return base64(sha256(bytes)).replaceAll("+", "-").replaceAll("/", "_").replace(/=+$/, ""); }
...
function strictlySorted(values) {
    for (let index = 1; index < values.length; index++) {
        ...
        if ((difference || left.length - right.length) >= 0)
            return false;
    }
    return true;
}
function canonicalOrigin(value) {
    ...
    if (match === null)
        return false;
    const host = match[1];
    if (host.length > 253 || !host.split(".").every((label) => label.length >= 1 && label.length <= 63 && /^[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/.test(label)))
        return false;
    const port = match[2];
    if (port !== undefined && (port === "443" || Number(port) > 65_535))
        return false;
