...
import { catalogEnvelopeFromCarrierTerm, catalogRotationEnvelopeFromCarrierTerm, catalogInventoryId, catalogRotationId, catalogServiceRealm, normalizeCatalogBootstrap, normalizeCatalogCutoff, transportCatalogId, verifyCatalogEnvelope, verifyCatalogRotationEnvelope, canonicalBytesForCatalogRotation } from "./treehouse_catalog_codec";
...
export async function prepareTreehouseCatalogInstallation(input) {
    try {
        ...
        if (!closed(value, ["review", "history", "store"]) || !reviewValid(value.review) ||
            !closed(value.store, ["kind", "expected"]) || value.store.kind !== "verified_fresh" || !tokenValid(value.store.expected))
            fail("malformed_catalog");
        const history = await observeHistory(value.history);
        if (history.raw.replica !== value.review.space)
            fail("wrong_catalog_scope");
        const observed = await treehouseCatalogBootstrapsFromFrames(history.raw);
        if (!observed.ok)
            fail("invalid_verified_history");
        const selected = observed.bootstraps.find((b) => b.id === value.review.bootstrapId);
        if (selected === undefined) {
            if (!history.byId.has(value.review.bootstrapId))
                fail("trust_pending", [value.review.bootstrapId]);
        ...
        if (selected.record.spaceRoot !== value.review.spaceRoot || selected.record.space !== value.review.space)
            fail("wrong_catalog_scope");
        const ids = sorted(observed.bootstraps.map((b) => b.id));
        if (!equal(ids, value.review.observedBootstrapIds))
            fail("catalog_fork", ids);
        ...
        if (!profile.ok || profile.root !== value.review.spaceRoot || profile.profileGenesis !== selected.record.profileGenesis ||
            profile.profileId !== selected.record.profileId)
            fail("wrong_catalog_scope");
        const next = { version: 1, review: value.review, histories: [history.raw],
...
export async function evaluateTreehouseCatalogTrust(input) {
    try {
        if (!closed(input, ["installed", "expected", "incoming"]))
            fail("malformed_catalog");
        let original;
        try {
            original = copyInput(ownValue(input, "installed"));
            if (!stateValid(original))
                fail("trust_recovery_required");
        }
        catch {
            fail("trust_recovery_required");
        }
        ...
        try {
            candidate = copyInput({ expected: ownValue(input, "expected"), incoming: ownValue(input, "incoming") });
        }
        catch (error) {
        ...
        if (checked.kind === "reject")
            throw new Refusal("trust_recovery_required", checked.detail);
...
async function evaluateSnapshot(value, originalOnly) {
    let frozen;
    try {
        if (!closed(value, ["installed", "expected", "incoming"]) || !tokenValid(value.expected) || !pageValid(value.incoming))
            fail("malformed_catalog");
        if (!stateValid(value.installed))
            fail("trust_recovery_required");
        ...
        if (bootstrap === null || bootstrap.space !== original.review.space || bootstrap.spaceRoot !== original.review.spaceRoot ||
            space.analysis.security.root?.pubkey !== original.review.spaceRoot)
            fail("trust_recovery_required");
        if (original.review.observedBootstrapIds.some((id) => space.byId.get(id)?.command !== "catalog_bootstrap_v1"))
            fail("trust_recovery_required");
        ...
        if (unseenBootstrapIds.length > 0 && next.blocked === null)
            next.blocked = block("catalog_fork", [], [], unseenBootstrapIds);
        if (next.blocked !== null)
            frozen = { next: structuredClone(next), expected: value.expected, replacement, bootstrapIds };
        const catalogs = new Map(), rotations = new Map();
        const retainedCatalogIds = new Set(original.catalogs.map((row) => row.id));
        for (const saved of original.catalogs)
            addCatalog(catalogs, saved.json, bootstrap, original.review.bootstrapId, saved.id);
        for (const saved of original.rotations)
            addRotation(rotations, saved.json, bootstrap, original.review.bootstrapId, saved.id);
        for (const json of value.incoming.catalogs)
            addCatalog(catalogs, json, bootstrap, original.review.bootstrapId);
        for (const json of value.incoming.rotations)
            addRotation(rotations, json, bootstrap, original.review.bootstrapId);
        const acceptedCatalogIds = new Set(), acceptedBindingIds = new Set();
        ...
        while (pendingCatalogs.length > 0 || pendingBindings.length > 0) {
            ...
            if (catalogId !== undefined && !acceptedCatalogIds.has(catalogId)) {
                ...
                if (node === undefined)
                    fail("trust_recovery_required");
                acceptedCatalogIds.add(catalogId);
                pendingBindings.push(node.envelope.catalog.binding);
                if (node.envelope.catalog.previous !== null)
                    pendingCatalogs.push(node.envelope.catalog.previous);
            }
            ...
            if (bindingId !== undefined && bindingId !== original.review.bootstrapId && !acceptedBindingIds.has(bindingId)) {
                ...
                if (node === undefined)
                    fail("trust_recovery_required");
                acceptedBindingIds.add(bindingId);
                pendingBindings.push(node.envelope.rotation.parent);
                pendingCatalogs.push(node.envelope.rotation.priorCatalog);
            }
        }
        const bindings = new Map([[original.review.bootstrapId, { id: original.review.bootstrapId, generation: 0,
                    key: bootstrap.catalogKey, parent: null, prior: null, pending: [], inventory: null }]]);
        const visiting = new Set();
        const catalogReady = new Set();
        const bindingFor = (id) => {
            const known = bindings.get(id);
            if (known !== undefined)
                return known;
            if (visiting.has(id))
                fail("invalid_catalog_transition", [id]);
            const node = rotations.get(id);
            if (node === undefined)
                fail("trust_pending", [id]);
            visiting.add(id);
            const r = node.envelope.rotation, parent = bindingFor(r.parent);
            if (r.newCatalogKey === bootstrap.serviceKey || r.generation !== parent.generation + 1 || !safe(parent.generation + 1))
                fail("invalid_catalog_transition", [id]);
            if (!verifyCatalogRotationEnvelope(node.envelope, parent.key)) {
                ...
                if (!ed25519.verify(old, bytes, pub, { zip215: false }))
                    fail("invalid_catalog_signature", [id]);
            ...
            if (prior.envelope.catalog.binding !== r.parent || catalogInventoryId(prior.envelope.catalog.entries) !== r.inventoryDigest ||
                !equal(r.cutoffs.map((c) => c.replica), prior.envelope.catalog.entries.map((e) => e.replica)))
                fail("invalid_catalog_transition", [id]);
            const binding = { id, generation: r.generation, key: r.newCatalogKey, parent: r.parent, prior: r.priorCatalog,
        ...
        const catalogFor = (id) => {
            ...
            if (node === undefined)
                fail("trust_pending", [id]);
            if (catalogReady.has(id))
                return node;
            if (visiting.has(id))
                fail("invalid_catalog_transition", [id]);
            visiting.add(id);
            const c = node.envelope.catalog, binding = bindingFor(c.binding);
            if (!verifyCatalogEnvelope(node.envelope, binding.key))
                fail("invalid_catalog_signature", [id]);
            node.authenticated = true;
            if (c.entries.some((e) => e.serviceId !== bootstrap.serviceId || e.serviceKey !== bootstrap.serviceKey))
                fail("wrong_catalog_scope", [id]);
            if (c.previous !== null) {
                const prior = catalogFor(c.previous).envelope.catalog;
                if (prior.binding !== c.binding || c.revision !== prior.revision + 1 || !safe(prior.revision + 1) || !inventoryExtends(prior.entries, c.entries))
                    fail("invalid_catalog_transition", [id]);
            }
            else if (binding.inventory !== null && catalogInventoryId(c.entries) !== binding.inventory)
                fail("invalid_catalog_transition", [id]);
            ...
            for (const entry of c.entries) {
                ...
                if (proof.invalid)
                    node.pending.push(entry.genesis, entry.creation, entry.reference);
                if (retainedDiagnostic)
                    node.pending.push(...proof.refused);
        ...
        for (const id of sorted(catalogs.keys()))
            catalogFor(id);
        for (const id of sorted(rotations.keys()))
            bindingFor(id);
        ...
        for (const [id, binding] of bindings) {
            ...
            for (const cutoff of r.cutoffs)
                if (!next.cutoffProofs.some((p) => equal(p.cutoff, cutoff)))
                    binding.pending.push(cutoff.logDigest);
            binding.pending = sorted(binding.pending);
        }
        for (const binding of [...bindings.values()].sort((a, b) => a.generation - b.generation)) {
            if (binding.parent !== null)
                binding.pending = sorted([...binding.pending, ...bindings.get(binding.parent).pending]);
        }
        ...
        for (const binding of bindings.values())
            if (binding.parent !== null) {
                ...
                for (const node of groups.get(binding.parent) ?? []) {
                    if (node.id !== prior && catalogAncestor(prior, node.id, catalogs))
                        forks.push(node.id, prior);
                }
            }
        const pendingIds = sorted([...catalogs.values()].filter((n) => n.pending.length > 0).map((n) => n.id));
        if (heads.length > 2 && next.blocked?.reason !== "authority_changed")
            next.blocked = block("control_history_limit", heads, forks, [], [], pendingIds);
        else if (next.blocked === null && (heads.length > 1 || forks.length > 0))
            next.blocked = block("catalog_fork", heads, forks, [], [], pendingIds);
        // Pending signed nodes are retained but their unproven entries reserve no route.
        const routesUsed = new Map();
        for (const node of catalogs.values())
            if (node.pending.length === 0)
                for (const entry of node.envelope.catalog.entries) {
                    const previous = routesUsed.get(entry.route);
                    if (previous !== undefined && previous !== entry.replica)
                        fail("invalid_catalog_transition", [node.id]);
        ...
        if (original.accepted !== null) {
            ...
            if (saved === undefined || binding === undefined || saved.envelope.catalog.binding !== binding.id ||
                ...
                saved.pending.length > 0 || binding.pending.length > 0)
                fail("trust_recovery_required");
        }
        if (originalOnly) {
            await validateStoredBlock(original, next.blocked, catalogs, bindings, histories);
            if (original.cutoffProofs.length !== next.cutoffProofs.length)
                fail("trust_recovery_required");
        ...
        const retained = [...original.catalogs.map((a) => ({ kind: "catalog", ...a })), ...original.rotations.map((a) => ({ kind: "rotation", ...a }))];
        const have = new Set(retained.map((a) => `${a.kind}:${a.id}`));
        const additions = [...next.catalogs.map((a) => ({ kind: "catalog", ...a })), ...next.rotations.map((a) => ({ kind: "rotation", ...a }))]
        ...
        if ((original.blocked?.triggers.length ?? 0) > 0 || overflow) {
            next.catalogs = original.catalogs;
            next.rotations = original.rotations;
            next.cutoffProofs = original.cutoffProofs;
            ...
            if (next.blocked?.reason !== "authority_changed")
        ...
        if (next.blocked !== null)
            return issue("retain_blocked", value.expected, next, next.blocked.reason, replacement, bootstrapIds, heads, catalogHeads, []);
        ...
        else if (latest === undefined || latest.pending.length > 0)
            reason = "trust_pending";
        else
            next.accepted = { binding: head.id, generation: head.generation, catalog: latest.id, revision: latest.envelope.catalog.revision };
        ...
        if (!originalOnly && frozen !== undefined)
            return issue("retain_blocked", frozen.expected, frozen.next, frozen.next.blocked.reason, frozen.replacement, frozen.bootstrapIds, frozen.next.blocked.bindings, [], []);
        return rejection(error);
    }
}
/** Candidate only: the adapter must persist the exact decision before using transport. */
export function resolveTreehouseCatalogRoute(input) {
    const held = decisions.get(input.decision);
    if (held === undefined)
        return { ok: false, reason: "trust_recovery_required" };
    if (held.blocked !== null)
        return { ok: false, reason: held.blocked };
...
const detail = (ids = [], coreReason = null, pendingProofIds = []) => ({ ids: sorted(ids), coreReason, pendingProofIds: sorted(pendingProofIds) });
class Refusal extends Error {
    reason;
    info;
    constructor(reason, info = detail()) {
        super(reason);
        this.reason = reason;
        this.info = info;
    }
}
function fail(reason, ids = []) { throw new Refusal(reason, detail(ids)); }
...
function issue(kind, expected, next, reason, replacementConfigured, bootstrapIds, bindingHeads, catalogHeads, routes) {
...
function block(reason, bindings, catalogs, bootstrapIds, opIds = [], pendingProofIds = []) {
...
function copyInput(input) {
    const pending = [{ value: input, leave: false }];
    const active = new Set(), finished = new Set();
    while (pending.length > 0) {
        const { value, leave } = pending.pop();
        if (leave && typeof value === "object" && value !== null) {
            active.delete(value);
            finished.add(value);
            continue;
        }
...
const text = (v) => typeof v === "string" && v.length > 0 && new TextDecoder("utf-8", { fatal: true, ignoreBOM: true }).decode(encoder.encode(v)) === v;
const idValid = (v) => typeof v === "string" && /^[A-Za-z0-9_-]{43}$/.test(v) && canonicalBase64Bytes(v.replaceAll("-", "+").replaceAll("_", "/") + "=", 32) !== null;
const sorted = (ids) => [...new Set(ids)].sort(compareUtf8);
...
const byId = (a, b) => compareUtf8(a.id, b.id);
function tokenValid(v) { return closed(v, ["trustRevision", "historyGeneration"]) && safe(v.trustRevision) && safe(v.historyGeneration) && v.trustRevision < Number.MAX_SAFE_INTEGER && v.historyGeneration < Number.MAX_SAFE_INTEGER; }
function reviewValid(v) {
...
function historyValid(v) {
    return closed(v, ["replica", "frames", "rejected"]) && text(v.replica) && Array.isArray(v.frames) && Array.isArray(v.rejected) &&
        v.rejected.every((r) => closed(r, ["frame", "reason"]) && r.reason === "bad_signature");
}
function pageValid(v) {
    if (!closed(v, ["catalogs", "rotations", "histories", "cutoffProofs"]) || !Array.isArray(v.catalogs) || !Array.isArray(v.rotations) || !Array.isArray(v.histories) || !Array.isArray(v.cutoffProofs))
        return false;
    if (v.catalogs.length + v.rotations.length > 32)
        fail("control_history_limit");
...
function stateValid(v) {
    if (!closed(v, ["version", "review", "histories", "catalogs", "rotations", "cutoffProofs", "accepted", "blocked"]) || v.version !== 1 || !reviewValid(v.review) ||
        !Array.isArray(v.histories) || !v.histories.every(historyValid) || !Array.isArray(v.catalogs) || !Array.isArray(v.rotations) || !Array.isArray(v.cutoffProofs))
        return false;
    if (![...v.catalogs, ...v.rotations].every((a) => closed(a, ["id", "json"]) && idValid(a.id) && typeof a.json === "string"))
        return false;
    if ([v.catalogs, v.rotations].some((a) => new Set(a.map((n) => n.id)).size !== a.length) ||
        v.catalogs.length + v.rotations.length > 1024 || [...v.catalogs, ...v.rotations].reduce((n, a) => n + encoder.encode(a.json).length, 0) > 16 * 1024 * 1024)
        return false;
    if (v.accepted !== null && (!closed(v.accepted, ["binding", "generation", "catalog", "revision"]) || !idValid(v.accepted.binding) || !idValid(v.accepted.catalog) || !safe(v.accepted.generation) || !safe(v.accepted.revision)))
        return false;
    if (v.blocked !== null && (!closed(v.blocked, ["reason", "bindings", "catalogs", "bootstrapIds", "opIds", "pendingProofIds", "triggers", "authorityWitnesses"]) ||
        !["catalog_fork", "authority_changed", "control_history_limit"].includes(v.blocked.reason) ||
        ...
        !v.blocked.triggers.every((t) => closed(t, ["kind", "id", "digest", "bytes"]) && (t.kind === "catalog" || t.kind === "rotation") && idValid(t.id) && idValid(t.digest) && safe(t.bytes))))
        return false;
    return true;
}
function validateStoredClosure(state) {
    for (const history of state.histories) {
        const ids = new Set(history.frames.map((frame) => frame.id));
        if (history.frames.some((frame) => frame.deps.some((id) => !ids.has(id))))
            fail("trust_recovery_required");
    }
    ...
    for (const artifact of state.catalogs) {
        ...
        if (!bindings.has(c.binding) || (c.previous !== null && !catalogs.has(c.previous)))
            fail("trust_recovery_required");
    }
    for (const artifact of state.rotations) {
        const r = catalogRotationEnvelopeFromCarrierTerm(parseJson(artifact.json))?.rotation;
        if (r === undefined || !bindings.has(r.parent) || !catalogs.has(r.priorCatalog))
            fail("trust_recovery_required");
    }
}
...
async function validateStoredBlock(state, derived, catalogs, bindings, histories) {
    ...
    if (![saved.bindings, saved.catalogs, saved.bootstrapIds, saved.opIds, saved.pendingProofIds].every((ids) => equal(ids, sorted(ids))) ||
        ...
        saved.pendingProofIds.some((id) => !catalogs.has(id)))
        fail("trust_recovery_required");
    ...
    if (saved.triggers.length > 0) {
        ...
        if ((saved.reason !== "control_history_limit" && saved.reason !== "authority_changed") ||
            ...
            saved.triggers.some((t) => t.bytes === 0 || t.bytes > 128 * 1024 || (t.kind === "catalog" ? catalogs : bindings).has(t.id)))
            fail("trust_recovery_required");
        ...
        if (count + saved.triggers.length <= 1024 && bytes + saved.triggers.reduce((n, t) => n + t.bytes, 0) <= 16 * 1024 * 1024)
            fail("trust_recovery_required");
    }
    if (saved.reason === "authority_changed") {
        if (saved.opIds.length === 0 || saved.bindings.length + saved.catalogs.length + saved.bootstrapIds.length + saved.pendingProofIds.length !== 0)
            fail("trust_recovery_required");
        await validateAuthorityWitnesses(state, catalogs, histories);
        return;
    }
    if (saved.authorityWitnesses.length > 0)
        fail("trust_recovery_required");
    if (saved.triggers.length > 0)
        return;
    if (saved.opIds.length > 0)
        fail("trust_recovery_required");
    await validateBootstrapWitnesses(state, histories);
    const bindingAncestor = (older, newer) => {
        let cursor = newer;
        while (cursor !== null) {
            if (cursor === older)
                return true;
            cursor = bindings.get(cursor).parent;
        }
        return false;
    };
    ...
    const catalogConflict = (a, b) => {
        const ca = catalogs.get(a).envelope.catalog, cb = catalogs.get(b).envelope.catalog;
        if (ca.binding !== cb.binding)
            return false;
        if (!catalogAncestor(a, b, catalogs) && !catalogAncestor(b, a, catalogs))
            return true;
    ...
    if (saved.catalogs.some((a) => !saved.catalogs.some((b) => a !== b && catalogConflict(a, b))))
        fail("trust_recovery_required");
    if (saved.reason === "control_history_limit") {
        if (!independentBindings || saved.bindings.length <= 2 || saved.bootstrapIds.length > 0)
            fail("trust_recovery_required");
    }
    else if (!independentBindings && saved.catalogs.length === 0 && saved.bootstrapIds.length === 0)
        fail("trust_recovery_required");
}
async function validateBootstrapWitnesses(state, histories) {
    const history = histories.get(state.review.space);
    if (history === undefined)
        fail("trust_recovery_required");
    const frames = new Map(history.raw.frames.map((frame) => [frame.id, frame]));
    for (const id of state.blocked.bootstrapIds) {
        if (state.review.observedBootstrapIds.includes(id))
            fail("trust_recovery_required");
        const closure = new Set(), pending = [id];
        while (pending.length > 0) {
        ...
        const observed = await treehouseCatalogBootstrapsFromFrames({ replica: state.review.space,
            frames: history.raw.frames.filter((frame) => closure.has(frame.id)) });
        if (!observed.ok || !observed.bootstraps.some((bootstrap) => bootstrap.id === id &&
            bootstrap.record.space === state.review.space && bootstrap.record.spaceRoot === state.review.spaceRoot))
            fail("trust_recovery_required");
    }
}
async function validateAuthorityWitnesses(state, catalogs, histories) {
    ...
    if (witnesses.length === 0 || !equal(witnesses.map((w) => w.replica), sorted(witnesses.map((w) => w.replica))) ||
        !equal(saved.opIds, sorted(witnesses.flatMap((w) => w.opIds))))
        fail("trust_recovery_required");
    const allowed = new Map([[state.review.space, new Set([state.review.bootstrapId])]]);
    if (state.accepted !== null)
        for (const entry of catalogs.get(state.accepted.catalog).envelope.catalog.entries) {
            const ids = allowed.get(entry.replica) ?? new Set();
            ids.add(entry.genesis);
            ids.add(entry.creation);
            allowed.set(entry.replica, ids);
    ...
    for (const witness of witnesses) {
        ...
        if (history === undefined || witness.frontier.length === 0 || witness.opIds.length === 0 ||
            ...
            witness.opIds.some((id) => !allowed.get(witness.replica)?.has(id)))
            fail("trust_recovery_required");
        const frames = new Map(history.frames.map((frame) => [frame.id, frame])), closure = new Set(), pending = [...witness.frontier];
        while (pending.length > 0) {
        ...
        const historical = await observeHistory({ replica: witness.replica, frames: history.raw.frames.filter((frame) => closure.has(frame.id)), rejected: [] });
        if (!equal(historical.frontier, witness.frontier) || witness.opIds.some((id) => !historical.projection.quarantineReasons.has(id)))
            fail("trust_recovery_required");
    }
}
async function observeHistory(raw) {
    if (!historyValid(raw))
        fail("invalid_verified_history");
    const family = continuationFamily(raw.replica);
    if (family !== "space" && family !== "thread")
        fail("wrong_catalog_scope");
    const missing = await authenticateFrames(raw);
    if (missing.length > 0)
        fail("trust_pending", missing);
    const ids = new Set(raw.frames.map((f) => f.id));
    const cutoff = await deriveTreehouseCatalogCutoff(raw);
    if (!cutoff.ok)
        fail(cutoff.reason);
    let frames;
    try {
        frames = raw.frames.map(decodeCarrierOpFrame);
    }
    catch {
        fail("unsupported_cutoff");
    }
    ...
    if (ops.length !== frames.length || order.length !== frames.length)
        fail("invalid_verified_history");
    return { raw: { ...raw, frames: [...raw.frames].sort((a, b) => compareUtf8(a.id, b.id)),
            rejected: [...raw.rejected].sort((a, b) => compareUtf8(a.frame.id, b.frame.id)) },
...
async function authenticateFrames(raw) {
    if (!historyValid(raw))
        fail("invalid_verified_history");
    const ids = new Set();
    ...
    for (const value of raw.frames) {
        const f = value;
        try {
            if (!text(f.id) || f.replica !== raw.replica || ids.has(f.id) || !Array.isArray(f.deps) || !f.deps.every(text))
                fail("invalid_verified_history");
            const bytes = canonicalBytesForCarrierOp(f), key = canonicalBase64Bytes(f.author, 32), sig = canonicalBase64Bytes(f.sig, 64);
            if (key === null || sig === null || await canonicalHash(bytes) !== f.id || !ed25519.verify(sig, bytes, key, { zip215: false }))
                fail("invalid_verified_history");
            ids.add(f.id);
        }
        catch (error) {
    ...
    return sorted(raw.frames.flatMap((f) => f.deps.filter((id) => !ids.has(id))));
}
async function mergeHistories(saved, incoming) {
    const map = new Map();
    for (const list of [saved, incoming]) {
        const seen = new Set();
        for (const history of list) {
            if (!historyValid(history) || seen.has(history.replica))
                fail("invalid_verified_history");
            seen.add(history.replica);
            const prior = map.get(history.replica);
            if (prior === undefined) {
                map.set(history.replica, history);
                continue;
            }
            const merge = (a, b, frame) => {
                const by = new Map();
                for (const items of [a, b]) {
                    const duplicates = new Set();
                    for (const item of items) {
                        const f = frame(item);
                        if (duplicates.has(f.id))
                            fail("invalid_verified_history");
                        duplicates.add(f.id);
                        const old = by.get(f.id);
                        if (old !== undefined && fingerprint(frame(old)) !== fingerprint(f))
                            fail("invalid_verified_history");
            ...
            map.set(history.replica, { replica: history.replica, frames: merge(prior.frames, history.frames, (v) => v),
                rejected: merge(prior.rejected, history.rejected, (v) => v.frame) });
        }
    }
    const missing = [];
    ...
    for (const raw of map.values()) {
        ...
        if (!rejected.ok && rejected.reason === "invalid_verified_history")
    ...
    for (const [id, raw] of map)
        observed.set(id, await observeHistory(raw));
    return observed;
}
function fingerprint(f) { return `${f.id}:${hash(canonicalBytesForCarrierOp(f))}:${f.sig}`; }
function hash(bytes) {
...
function parseJson(json) {
    if (typeof json !== "string")
        fail("malformed_catalog");
    if (encoder.encode(json).length > 128 * 1024)
        fail("control_history_limit");
    try {
...
function addCatalog(map, json, bootstrap, bootstrapId, savedId) {
    const envelope = parseCatalog(json), id = transportCatalogId(envelope.catalog);
    if (savedId !== undefined && savedId !== id)
        fail("trust_recovery_required");
    if (envelope.catalog.space !== bootstrap.space || envelope.catalog.bootstrap !== bootstrapId)
        fail("wrong_catalog_scope", [id]);
    if (!map.has(id))
        map.set(id, { id, json, envelope, authenticated: false, pending: [] });
    else if (map.get(id).envelope.signature !== envelope.signature)
        fail("invalid_catalog_signature", [id]);
}
function addRotation(map, json, bootstrap, bootstrapId, savedId) {
    ...
    if (savedId !== undefined && savedId !== id)
        fail("trust_recovery_required");
    if (envelope.rotation.space !== bootstrap.space || envelope.rotation.bootstrap !== bootstrapId)
        fail("wrong_catalog_scope", [id]);
    if (!map.has(id))
        map.set(id, { id, json, envelope, authenticated: false });
}
function inventoryExtends(before, after) {
    return before.every((entry) => {
        const next = after.find((e) => e.replica === entry.replica);
        return next !== undefined && ["root", "genesis", "creation", "reference", "kind", "schema"].every((key) => entry[key] === next[key]);
    });
}
function ancestor(older, newer, by) {
    const seen = new Set(), pending = [...(by.get(newer)?.deps ?? [])];
    while (pending.length > 0) {
        const id = pending.pop();
        if (id === older)
            return true;
        if (seen.has(id))
            continue;
        seen.add(id);
        pending.push(...(by.get(id)?.deps ?? []));
    }
    return false;
}
function entryProof(entry, histories, space) {
    const h = histories.get(entry.replica), result = { pending: [], refused: [], invalid: false };
    if (continuationFamily(entry.replica) !== entry.kind)
        return { ...result, invalid: true };
    if (h === undefined)
        return { ...result, pending: [entry.genesis, entry.creation, entry.reference] };
    ...
    for (const [id, op, history] of [[entry.genesis, genesis, h], [entry.creation, creation, h], [entry.reference, reference, space]]) {
        if (op === undefined)
            result.pending.push(id);
        else if (history.projection.quarantineReasons.has(id))
            result.refused.push(id);
    }
    if (h.analysis.security.root?.pubkey !== entry.root)
        result.invalid = true;
    if (genesis !== undefined) {
        ...
        if (genesis.kind !== "authority" || d === undefined || d.parentId !== null || d.issuer !== entry.root || d.audience !== entry.root ||
            genesis.authorPubkey !== entry.root || !h.analysis.security.delegations.get(d.id)?.validation.valid)
            result.invalid = true;
    }
    if (creation !== undefined && (creation.kind !== "command" || creation.command !== (entry.kind === "space" ? "create_space" : "create_thread") ||
        (genesis !== undefined && !ancestor(genesis.id, creation.id, h.byId))))
        result.invalid = true;
    if (reference !== undefined && (reference.kind !== "command" || (entry.kind === "space" ? reference.command !== "catalog_bootstrap_v1" || entry.replica !== space.raw.replica :
        reference.command !== "create_thread" || reference.commandArgs?.length !== 2 || reference.commandArgs[0] !== entry.replica)))
        result.invalid = true;
    return result;
}
async function mergeProofs(saved, incoming, histories) {
    const proofs = new Map();
    for (const proof of [...saved, ...incoming]) {
        if (!closed(proof, ["cutoff", "history"]) || normalizeCatalogCutoff(proof.cutoff) === null || !historyValid(proof.history))
            fail("malformed_catalog");
        const observed = await deriveTreehouseCatalogCutoff(proof.history);
        if (!observed.ok)
            fail(observed.reason);
        if (!equal(observed.cutoff, proof.cutoff))
            fail("recovery_incomplete", [proof.cutoff.logDigest]);
        const current = histories.get(proof.history.replica);
        if (current === undefined)
            fail("recovery_incomplete", [proof.cutoff.logDigest]);
        const present = new Set(current.raw.frames.map((f) => fingerprint(f)));
        const rejected = new Set(current.raw.rejected.map((r) => fingerprint(r.frame)));
        if (proof.history.frames.some((f) => !present.has(fingerprint(f))) || proof.history.rejected.some((r) => !rejected.has(fingerprint(r.frame))))
            fail("recovery_incomplete", [proof.cutoff.logDigest]);
...
function catalogAncestor(older, newer, nodes) {
    let cursor = newer;
    const seen = new Set();
    while (cursor !== null && !seen.has(cursor)) {
        if (cursor === older)
            return true;
        seen.add(cursor);
        cursor = nodes.get(cursor)?.envelope.catalog.previous ?? null;
    }
    return false;
}
function catalogRoutes(node, binding, bootstrap) {
