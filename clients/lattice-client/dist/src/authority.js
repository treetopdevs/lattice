...
export function analyzeAuthority(schema, ops, included, order, byId, expectedReplica = undefined) {
    ...
    const roleViews = (op) => op.kind === "authority" ? effectViews(op) : [op];
    ...
    for (const original of admitted) {
        ...
        if (original.kind === "authority" && original.effects !== undefined) {
            ...
            if (quarantineReasons.has(original.id))
                continue;
            const denied = views.find((view) => authorityRoleWrite(schema, view) &&
                view.authority !== undefined && !authorityWriteHonored(view, view.authority, states.get(view.field) ?? emptyRoleState(), delegations, policies, honoredSuccessionIntroductions, byId));
            if (denied !== undefined && denied.authority !== undefined) {
                const reason = authorityWriteRejectionReason(denied, denied.authority, states.get(denied.field) ?? emptyRoleState(), delegations, policies, honoredSuccessionIntroductions, byId);
                if (reason !== undefined)
                    quarantineReasons.set(original.id, reason);
