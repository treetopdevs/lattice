...
const digest = (label) => createHash("sha256").update(`r19b-policy:${label}`).digest();
const b64 = (bytes) => Buffer.from(bytes).toString("base64");
const opId = (label) => digest(label).toString("base64url");
const identity = (label) => ({ seed: digest(label), pub: ed25519.getPublicKey(digest(label)) });
...
let replica = "";
function op(label, command, args, deps = []) {
    const id = opId(label);
...
function admission(label, key, deps = []) {
    return op(label, "admit_member", ["invite", b64(key), "member", "acceptance"], deps);
}
function removal(label, key, deps) {
    return op(label, "remove_member", [b64(key)], deps);
}
function beacon(label, deps = []) {
    const value = op(label, "beacon", [], deps);
    return { ...value, kind: "authority", authority: { type: "beacon", epoch: 0 } };
}
async function fixture() {
    ...
    const claim = { version: 1, product: "treehouse", space: replica,
    ...
    const certificate = { claim, possession, vouches: voucherKeys.map((key) => ({ member: b64(key.pub),
    ...
    const context = { visibleOps, verdicts, validBeacons: [{ opId: epoch.id, epoch: 0 }] };
...
test("individual judgment follows shape, target, eligibility, epoch, context, certificate precedence", async () => {
    ...
    assert.deepEqual(treehouseCommandOpStatus(treehouseSpaceSchema, f.statement, new Set(f.context.visibleOps.keys()), f.context), { ok: true });
    const malformed = { ...f.statement, commandArgs: [{ ...f.claim, extra: true }, f.certificate.possession, f.certificate.vouches] };
    assert.equal(reason(memberContinuityCommandStatus(malformed, new Set(), { visibleOps: new Map(), verdicts: new Map(), validBeacons: [] })), "application_invalid_continuity");
    const missing = new Map(f.context.visibleOps);
    missing.delete(f.vouchers[0].id);
    const quarantined = new Map(f.context.verdicts);
    quarantined.set(f.oldAdmission.id, "bad_signature");
    assert.equal(reason(memberContinuityCommandStatus(f.statement, new Set(missing.keys()), { ...f.context, visibleOps: missing })), "application_target_not_visible");
    assert.equal(reason(memberContinuityCommandStatus(f.statement, new Set(f.context.visibleOps.keys()), { ...f.context, verdicts: quarantined })), "application_target_quarantined");
    ...
    assert.equal(reason(memberContinuityCommandStatus(f.statement, new Set(wrongOps.keys()), { ...f.context, visibleOps: wrongOps })), "application_wrong_target");
    ...
    assert.equal(reason(memberContinuityCommandStatus(f.statement, new Set(removedOps.keys()), { ...f.context, visibleOps: removedOps, verdicts: removedVerdicts })), "application_continuity_ineligible_member");
    assert.equal(reason(memberContinuityCommandStatus(f.statement, new Set(f.context.visibleOps.keys()), { ...f.context, validBeacons: [] })), "application_continuity_invalid_epoch");
    assert.equal(reason(memberContinuityCommandStatus(f.statement, new Set(f.context.visibleOps.keys()), { ...f.context, validBeacons: [{ opId: f.claim.epochBasis[0], epoch: "9007199254740993" }] })), "application_continuity_invalid_epoch");
    const prior = op("prior-wrapper", "attest_member_key_v1", [f.claim, f.certificate.possession, f.certificate.vouches], f.claim.deps);
    ...
    assert.equal(reason(memberContinuityCommandStatus(f.statement, new Set(priorOps.keys()), { ...f.context, visibleOps: priorOps, verdicts: priorVerdicts })), "application_continuity_stale_context");
    const bad = { ...f.statement, commandArgs: [f.claim, b64(new Uint8Array(64)), f.certificate.vouches] };
    assert.equal(reason(memberContinuityCommandStatus(bad, new Set(f.context.visibleOps.keys()), f.context)), "application_continuity_invalid_certificate");
});
...
test("deny-only graph seeds stale vouchers before target collision and propagates invalid parents", async () => {
    ...
    const otherVouches = voucherKeys.map((key) => ({ member: b64(key.pub), signature: b64(ed25519.sign(codec.canonicalBytesForMemberContinuityVouch(otherClaim, otherPossession), key.seed)) }));
    ...
    const resolutionVouches = voucherKeys.map((key) => ({ member: b64(key.pub), signature: b64(ed25519.sign(codec.canonicalBytesForMemberContinuityVouch(resolutionClaim, resolutionPossession), key.seed)) }));
    ...
    const collisions = memberContinuityCommandConflicts(collisionOps, new Map([...collisionOps.keys()].map((id) => [id, "honored"])));
    ...
    const parents = memberContinuityCommandConflicts(parentOps, new Map([...parentOps.keys()].map((id) => [id, "honored"])));
    ...
    const freshVouches = voucherKeys.map((key) => ({ member: b64(key.pub), signature: b64(ed25519.sign(codec.canonicalBytesForMemberContinuityVouch(freshClaim, freshPossession), key.seed)) }));
    ...
    assert.deepEqual(memberContinuityCommandStatus(fresh, new Set(causalOps.keys()), { ...f.context, visibleOps: causalOps, verdicts: causalVerdicts }), { ok: true });
});
...
test("public observation authenticates raw closure and uses the ordinary capability judge", async () => {
    ...
    raw.frames.push(pin);
    assert.deepEqual(await observeMemberContinuityFromFrames({ replica: "replica:treehouse:thread:unsupported", frames: [] }), { ok: false, reason: "unsupported_continuity_history" });
    assert.deepEqual(await observeMemberContinuityFromFrames({ replica: raw.replica, frames: raw.frames, oldPub: "not-a-key" }), { ok: false, reason: "invalid_verified_history" });
    ...
    assert.deepEqual(await observeMemberContinuityFromFrames({ replica: raw.replica, frames: forged }), { ok: false, reason: "invalid_verified_history" });
    const partial = raw.frames.filter((frame) => frame.id === raw.command.id);
    assert.deepEqual(await observeMemberContinuityFromFrames({ replica: raw.replica, frames: partial }), { ok: false, reason: "invalid_verified_history" });
    ...
    if (ordered.ok)
        assert.deepEqual(ordered.links.map((link) => link.oldPub), [rawFirst, base64First]
...
test("review derives consent from signed history and assembly never invokes a signer before public judgment", async () => {
    ...
    const adminSigner = { publicKey: admin.pub, sign: (bytes) => ed25519.sign(bytes, admin.seed) };
    ...
    for (const members of [[admissions[0], admissions[2]], [admissions[1], admissions[2]]]) {
    ...
    assert.deepEqual(await reviewMemberContinuityFromFrames({ replica: genesis.replica,
        ...
        voucherAdmissions: [admissions[1], duplicateAdmission.id], author: b64(admin.pub), capId: delegation.id }), { ok: false, reason: "application_invalid_continuity" });
    ...
    const request = { replica: genesis.replica, frames, oldPub: b64(former.pub), oldAdmission: admissions[0],
        newPub: b64(successor.pub), oldMembership: "active", nonce: b64(digest("integration-nonce")),
        voucherAdmissions: [admissions[1], admissions[2]], author: b64(admin.pub), capId: delegation.id };
    const reviewed = await reviewMemberContinuityFromFrames(request);
    assert.equal(reviewed.ok, true, JSON.stringify(reviewed));
    assert.deepEqual(await reviewMemberContinuityFromFrames({ ...request, oldMembership: "removed" }), { ok: false, reason: "application_continuity_ineligible_member" });
    assert.deepEqual(await reviewMemberContinuityFromFrames({ ...request, capId: opId("unknown-cap") }), { ok: false, reason: "no_capability" });
    ...
    assert.deepEqual(await reviewMemberContinuityFromFrames({ ...request, author: aliasedAuthor }), { ok: false, reason: "application_invalid_continuity" });
    assert.deepEqual(await reviewMemberContinuityFromFrames({ ...request, capId: request.capId + "=" }), { ok: false, reason: "application_invalid_continuity" });
    if (!reviewed.ok)
        return;
    ...
    assert.deepEqual(await assembleMemberContinuityFromFrames({ frames, review: alteredConsent, certificate, signer: guarded }), { ok: false, reason: "stale_verified_state" });
    assert.equal(calls, 0);
    ...
    if (assembled.ok) {
        ...
        assert.equal(observed.ok, true);
        if (observed.ok)
            assert.equal(observed.records[0]?.claimId, reviewed.review.claimId);
    }
    let malformedCalls = 0;
    const malformedSigner = { publicKey: admin.pub, sign: () => { malformedCalls++; return new Uint8Array(63); } };
    assert.deepEqual(await assembleMemberContinuityFromFrames({ frames, review: reviewed.review, certificate, signer: malformedSigner }), { ok: false, reason: "invalid_signer_signature" });
    ...
    const reviewedGenesisSignature = reviewed.review.request.frames[0].sig;
    request.frames[0].sig = b64(new Uint8Array(64));
    ...
    assert.equal(reviewed.review.request.frames[0].sig, reviewedGenesisSignature);
    assert.notEqual(reviewed.review.request.voucherAdmissions[0], request.voucherAdmissions[0]);
});
async function profilePin(genesis, signer) {
