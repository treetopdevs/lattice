...
const digest = (name) => createHash("sha256").update(name).digest("base64url");
const key = (seed) => Buffer.from(ed25519.getPublicKey(Buffer.alloc(32, seed))).toString("base64");
const sign = (bytes, seed) => Buffer.from(ed25519.sign(bytes, Buffer.alloc(32, seed))).toString("base64");
const bootstrap = {
...
const envelope = { catalog, signature: sign(catalogCodec.canonicalBytesForTransportCatalog(catalog), 2) };
...
test("all seven public shapes round-trip exact fields and reject missing or extra typed and raw fields", () => {
    for (const [value, normalize, encode, decode] of shapes) {
        ...
        for (const extra of [{ ...value, unknown: true }, { ...value, [Symbol("extra")]: true }])
            assert.equal(normalize(extra), null);
        for (const invalid of [null, undefined, [], new Map(), Object.create(value)])
...
test("typed and decoded keys, signatures and digest IDs require canonical bytes and exact sizes", () => {
    for (const field of ["spaceRoot", "catalogKey", "serviceKey"]) {
    ...
    for (const field of ["profileGenesis", "profileId", "serviceId", "nonce"]) {
    ...
    for (const [field, value] of [
        ...
        assert.equal(catalogCodec.catalogBootstrapFromCarrierTerm(changeField(encoded, field, value)), null);
});
test("text preserves Unicode and BOM while refusing lossy surrogate or invalid UTF8 input", () => {
    ...
    for (const space of ["", "space:\ud800", "space:\udfff"])
        assert.equal(catalogCodec.normalizeCatalogBootstrap({ ...bootstrap, space }), null);
    const term = catalogCodec.catalogBootstrapToCarrierTerm(bootstrap);
    for (const bytes of ["/w==", "wK8=", "7aCA"])
        assert.equal(catalogCodec.catalogBootstrapFromCarrierTerm(changeField(term, "space", ["bin", bytes])), null);
});
...
test("revision and rotation integers are portable and first-snapshot nil is exact", () => {
    ...
    for (const previous of [["atom", "nil"], ["nil", "extra"], null])
        assert.equal(catalogCodec.transportCatalogFromCarrierTerm(changeField(term, "previous", previous)), null);
...
test("catalog signature uses only the caller's key and refuses tampering or an envelope-chosen signer", () => {
    ...
    for (const suppliedKey of [key(3), key(2).slice(0, -1), null])
        assert.equal(catalogCodec.verifyCatalogEnvelope(envelope, suppliedKey), false);
    for (const value of [
...
function text(value) { return ["bin", Buffer.from(value).toString("base64")]; }
function changeField(term, field, replacement) {
    assert.equal(term[0], "map");
    if (term[0] !== "map")
        throw new Error("expected map");
