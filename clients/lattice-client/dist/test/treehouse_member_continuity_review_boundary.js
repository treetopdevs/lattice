...
import { reviewMemberContinuityFromFrames as review, observeMemberContinuityFromFrames as observe } from "../src/treehouse_member_continuity";
...
test("review canonicalizes both voucher admission orders to identical consent bytes", async () => {
    ...
    const second = await review({ ...input, voucherAdmissions: [...input.voucherAdmissions].reverse() });
    assert.ok(first.ok);
...
test("an exact Space namespace without a root-authenticated pin is unsupported", async () => {
    const input = request();
    const genesis = input.frames.find(frame => frame.deps.length === 0);
    ...
    const genuinePin = input.frames.find(frame => frame.kind === "authority" && frame.deps.includes(genesis.id));
...
test("closed review input refuses extra authority fields and non-tuple vouchers before history", async () => {
    ...
    for (const vouchers of [input.voucherAdmissions[0], new Set(input.voucherAdmissions), [], [input.voucherAdmissions[0]],
        ...
        assert.deepEqual(await review({ ...input, voucherAdmissions: vouchers }), refusal("application_invalid_continuity"));
    }
});
test("signed missing and quarantined admission references preserve target precedence", async () => {
    ...
    for (const target of [input.oldAdmission, input.voucherAdmissions[0]]) {
        ...
        const selected = target === input.oldAdmission ? { oldAdmission: invalid.id } : { voucherAdmissions: [invalid.id, input.voucherAdmissions[1]] };
