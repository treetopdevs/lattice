...
const id = (label) => createHash("sha256").update(`r11a-beam-${label}`).digest("base64url");
const signer = (label) => {
...
test("package entry keeps the semantic binding predicate internal and preserves all existing authority values", () => {
    ...
    for (const name of existing)
        assert.equal(publicApi[name], authority[name], name);
...
async function fixture(replicaName = `replica:treehouse:space:${id("history-space")}#authority:bounded-continuation-v1`) {
    ...
    const profile = { mode: "bounded_continuation", version: 1, product: "treehouse", kind: "space", role: "admin",
    ...
    const record = { version: 1, product: "treehouse", space: replica, spaceRoot: b64(root.publicKey),
...
async function authenticated(frames) {
...
function bootstrap(f, record, overrides = {}) {
...
test("closed bootstrap shape failures remain signed application refusals", async () => {
    ...
    const change = (field, value) => ["map", raw[1].map(([key, old]) => [key, key[0] === "atom" && key[1] === field ? value : old])];
    const invalid = [nil, ["list", []], ["map", raw[1].slice(1)], ["map", [...raw[1], [atom("extra"), nil]]],
...
test("root-only preparation preserves original signed bytes and six-command capability ceiling", async () => {
    ...
    const f = await fixture();
    const bad = await bootstrap(f, catalogBootstrapToCarrierTerm({ ...f.record, space: expected.replica }), { replica: expected.replica, deps: [prepared.pending[1].id], cap: townshipCapTerm(d.id) });
...
test("internal profile binding rejects incomplete, duplicate, cyclic and mismatched semantic inputs", async () => {
    ...
    const matches = (history, replica = f.replica, root = f.record.spaceRoot) => continuationProfileBindingMatches(replica, history, root, f.pin.id, f.record.profileId);
    ...
    for (const history of [ops.slice(1), [...ops, ops[0]], ops.map((op, i) => i === 0 ? { ...op, deps: [f.pin.id] } : op),
        ops.map((op, i) => i === 0 ? { ...op, replica: "other" } : op)])
        assert.equal(matches(history), false);
...
test("flat bootstrap adapter preserves raw codec meaning and rejects duplicate pairs and non-scalar sentinels", async () => {
    ...
    const leaf = (term) => {
        ...
        if (term[0] === "int" && typeof term[1] === "number")
            return term[1];
        if (term[0] === "bin") {
            const bytes = Buffer.from(term[1], "base64");
            return { type: "bin", bytes, text: new TextDecoder().decode(bytes) };
        }
        throw new Error("fixture is not flat");
    };
    const decoded = { type: "map", pairs: raw[1].map(([key, value]) => [leaf(key), leaf(value)]) };
    ...
    assert.equal(catalogBootstrapFromDecodedTerm({ ...decoded, pairs: decoded.pairs.map((pair, i) => i === 0 ? [...pair, null] : pair) }), null);
    for (const value of [null, { type: "list", values: [] }, { type: "invalid_reserved_metadata" }]) {
    ...
    assert.equal(catalogBootstrapFromDecodedTerm(wrongBytes), null);
    const duplicate = ["map", [...raw[1], raw[1][0]]];
    ...
    assert.deepEqual(await treehouse.treehouseCatalogBootstrapsFromFrames({ replica: f.replica, frames: [...f.before, altered] }), { ok: false, reason: "invalid_verified_history" });
});
...
test("valid outer signatures cannot authorize an unsupported family or impostor profile introduction", async () => {
    for (const name of [`replica:treehouse:space:${id("unbound-family")}`, `replica:treehouse:thread:${id("wrong-family")}#authority:bounded-continuation-v1`]) {
        ...
        assert.deepEqual(await treehouse.treehouseCatalogBootstrapsFromFrames({ replica: f.replica, frames: f.frames }), { ok: true, bootstraps: [], verifiedFrontier: [f.bootstrap.id] });
    }
    const f = await fixture();
    ...
    assert.deepEqual(await treehouse.treehouseCatalogBootstrapsFromFrames({ replica: f.replica, frames }), { ok: true, bootstraps: [], verifiedFrontier: [command.id] });
});
