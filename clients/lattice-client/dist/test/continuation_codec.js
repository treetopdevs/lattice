...
import { canonicalBytesForContinuationClaim, canonicalBytesForContinuationProfile, continuationCertificateFromCarrierTerm, continuationCertificateToCarrierTerm, continuationClaimFromCarrierTerm, continuationClaimToCarrierTerm, continuationProfileFromCarrierTerm, continuationProfileId, continuationProfileToCarrierTerm, normalizeContinuationCertificate, normalizeContinuationClaim, normalizeContinuationProfile, verifyContinuationCertificate, } from "../src/continuation";
...
test("claims reject unknown fields, unsafe epochs, noncanonical IDs and reordered dependencies", () => {
    ...
    for (const epoch of [0, Number.MAX_SAFE_INTEGER])
        assert.ok(normalizeContinuationClaim({ ...claim, epoch }));
...
test("canonical bytes use the new domains, snake-case atom fields and raw key bytes", () => {
    ...
    const expectedProfilePairs = [
        ...
        [atomTerm("witnesses"), ["list", normalized.witnesses.map((key) => ["bin", key])]],
        [atomTerm("threshold"), ["int", 2]], [atomTerm("max_lease_epochs"), ["int", 7]],
    ];
    const expectedProfileTerm = ["map", expectedProfilePairs];
...
test("carrier decoding refuses duplicate, unknown and non-atom keys and malformed nested terms", () => {
    ...
    if (profileTerm[0] !== "map")
        throw new Error("expected profile map");
    ...
    const claimTerm = continuationClaimToCarrierTerm(claim);
    if (claimTerm[0] !== "map")
        throw new Error("expected claim map");
    for (const [field, bad] of [
        ...
        const changed = ["map", claimTerm[1].map(([key, value]) => [key, key[0] === "atom" && key[1] === field ? bad : value])];
    ...
    if (certificateTerm[0] !== "map")
        throw new Error("expected certificate map");
    assert.equal(continuationCertificateFromCarrierTerm(["map", [certificateTerm[1][0], certificateTerm[1][0]]]), null);
});
function atomTerm(value) { return ["atom", value]; }
function textTerm(value) { return ["bin", Buffer.from(value).toString("base64")]; }
