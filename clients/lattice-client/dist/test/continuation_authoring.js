...
const schema = { name: "ContinuationAuthoringFixture", fields: { admin: { authority: "admin" } } };
const atom = (name) => ["atom", name];
const tuple = (...values) => ["tuple", values];
const nil = ["nil"];
const b64 = (bytes) => Buffer.from(bytes).toString("base64");
const signer = (seedByte) => {
    const seed = new Uint8Array(32).fill(seedByte);
    return { publicKey: ed25519.getPublicKey(seed), sign: (bytes) => ed25519.sign(bytes, seed) };
};
async function fixture(familyVersion = "bounded-continuation-v1", kind = "space") {
    const role = kind === "space" ? "admin" : "moderator";
    const roleSchema = { name: schema.name, fields: { [role]: { authority: role } } };
    ...
    const replica = await bindTownshipReplica(`replica:treehouse:${kind}:${Buffer.alloc(32, 11).toString("base64url")}#authority:${familyVersion}`, root.publicKey);
    const profile = { mode: "bounded_continuation", version: 1, product: "treehouse",
...
async function certificate(review, witnesses) {
...
test("fresh assembly signs a normal op honored by the public authority judge", async () => {
    ...
    assert.equal((await verifyCarrierOp(result.frame, { verify: async (author, bytes, sig) => ed25519.verify(sig, bytes, Buffer.from(author, "base64"), { zip215: false }) })).valid, true);
...
test("raw history refuses wrong hashes, signatures, replicas, duplicates and incomplete closure", async () => {
    ...
    for (const frames of [
        ...
        assert.deepEqual(await reviewContinuationFromFrames({ ...f.input, frames }), { ok: false, reason: "invalid_verified_history" });
});
test("authenticated but malformed authority stays in the reviewed frontier", async () => {
    ...
    const malformed = await authorCarrierOp({ replica: f.replica, deps: [f.beacon.id],
        ...
        body: tuple(atom("succeed"), atom("admin"), atom("missing_delegation"), tuple(atom("continuation_v1"), ["map", []])) });
...
test("unknown signed family and display aliases cannot create an authorized review", async () => {
    ...
    assert.deepEqual(await reviewContinuationFromFrames(unknown.input), { ok: false, reason: "unsupported_authority_profile" });
    const f = await fixture();
    assert.deepEqual(await reviewContinuationFromFrames({ ...f.input, author: "holder" }), { ok: false, reason: "unauthorized_continuation" });
});
test("review authenticates a new delegation and enforces inherited scope and the finite window", async () => {
    ...
    for (const delegation of [
        ...
        assert.deepEqual(await reviewContinuationFromFrames({ ...f.input, delegation }), { ok: false, reason: "unauthorized_continuation" });
    for (const changes of [
        ...
        assert.deepEqual(await reviewContinuationFromFrames({ ...f.input, delegation }), { ok: false, reason: "continuation_scope_exceeded" }, JSON.stringify(changes));
    }
    ...
    assert.deepEqual(await reviewContinuationFromFrames({ ...f.input, delegation: unleased }), { ok: false, reason: "continuation_scope_exceeded" });
    const malformed = { ...f.delegation, expires_epoch: null };
    assert.deepEqual(await reviewContinuationFromFrames({ ...f.input, delegation: malformed }), { ok: false, reason: "malformed_term" });
});
test("only the actual frozen frontier can be reviewed", async () => {
    const f = await fixture();
    for (const deps of [[], [f.pin.id], [f.beacon.id, f.beacon.id]]) {
        assert.deepEqual(await reviewContinuationFromFrames({ ...f.input, deps }), { ok: false, reason: "stale_verified_state" });
    }
...
test("changed current frontier or caller facts refuse before invoking the signer", async () => {
    ...
    let calls = 0;
    const guarded = { publicKey: f.holder.publicKey, sign: (bytes) => {
            calls++;
            return f.holder.sign(bytes);
        } };
...
test("certificate replay against a new delegation, invalid surplus, and wrong signer refuse", async () => {
    ...
    assert.deepEqual(await assembleContinuationFromFrames({ schema, frames: f.input.frames,
        review: changed.review, certificate: cert, signer: guarded }), { ok: false, reason: "invalid_continuation_certificate" });
    for (const bad of [
    ...
    assert.deepEqual(await assembleContinuationFromFrames({ schema, frames: f.input.frames,
        review, certificate: cert, signer: { ...guarded, publicKey: f.nominee.publicKey } }), { ok: false, reason: "wrong_signer" });
    assert.equal(calls, 0);
});
...
test("Thread moderator review and assembly use the explicit role schema", async () => {
    ...
    assert.deepEqual(await reviewContinuationFromFrames({ ...f.input, schema }), { ok: false, reason: "unauthorized_continuation" });
});
...
test("signer failures and invalid returned signatures fail closed", async () => {
    ...
    assert.deepEqual(await assembleContinuationFromFrames({ schema, frames: f.input.frames, review,
        certificate: cert, signer: { publicKey: f.holder.publicKey, sign: () => new Uint8Array(64) } }), { ok: false, reason: "invalid_signer_signature" });
    assert.deepEqual(await assembleContinuationFromFrames({ schema, frames: f.input.frames, review,
