...
// --- Case A: forged transfer by a non-holder MUST be refused, not honored ---
{
    ...
    check("forged transfer by non-holder is refused (throws)", threw instanceof V01UnvalidatedAuthorityError, threw === null
...
// bypassing the V-01 refusal (caught by township_feed.ts:562 in CI).
{
    ...
    check("anchor-less evidence-free authority write is refused under a pinned anchor (throws)", threw instanceof V01UnvalidatedAuthorityError, threw === null
...
// judgeable foreign evidence: quarantined as wrong_replica, reduction proceeds.
{
    ...
    check("foreign-replica evidence-free write is quarantined as wrong_replica", !!result && result.quarantine.includes("x") && result.quarantineReasons.get("x") === "wrong_replica", result ? `quarantine=${JSON.stringify(result.quarantine)} reason=${result.quarantineReasons.get("x")}` : "no result");
    check("anchored genesis holder survives the foreign write", !!result && result.state.clerk === "clerk", result ? `clerk=${JSON.stringify(result.state.clerk)}` : "no result");
}
