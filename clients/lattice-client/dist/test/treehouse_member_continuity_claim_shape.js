...
const digest = (label) => createHash("sha256").update(`ts-semantic-independent:${label}`).digest();
const key = (label) => { const seed = digest(label); return { publicKey: ed25519.getPublicKey(seed), sign: (bytes) => ed25519.sign(bytes, seed) }; };
...
const originalArgs = codec.memberContinuityCommandArgumentsToCarrierTerm(original)[1];
function signed(args, deps = original.claim.deps) {
...
async function resolution(parent, additional = []) {
    ...
    return signed(codec.memberContinuityCommandArgumentsToCarrierTerm(certificate)[1], claim.deps);
}
const cases = Promise.all([
    { name: "four_arguments", terms: [...originalArgs, ["nil"]], hasClaim: false },
    ...
    { name: "malformed_possession", terms: [originalArgs[0], ["bin", ""], originalArgs[2]], hasClaim: true },
    { name: "malformed_vouch_list", terms: [originalArgs[0], originalArgs[1], ["list", []]], hasClaim: true },
    { name: "malformed_vouch_type", terms: [originalArgs[0], originalArgs[1], ["nil"]], hasClaim: true },
...
for (const name of ["four_arguments", "one_argument", "two_arguments", "malformed_possession", "malformed_vouch_list", "malformed_vouch_type"]) {
    test(`signed ${name} preserves BEAM exact-three-argument observation and target precedence`, async () => {
        ...
        assert.ok(parent.ok, JSON.stringify(parent));
        assert.deepEqual(parent.records, []);
        ...
        for (const frames of [[...parents, entry.child], [...parents, entry.child].reverse()]) {
            ...
            assert.equal(observed.quarantine.find(item => item.opId === entry.child.id)?.reason, entry.hasClaim ? "application_target_quarantined" : "application_target_not_visible");
        }
    });
}
...
test("optional authenticated parity export contains untouched base and independently signed edge histories", async () => {
    if (process.env.TREEHOUSE_CONTINUITY_SHAPE_EXPORT)
        writeFileSync(process.env.TREEHOUSE_CONTINUITY_SHAPE_EXPORT, JSON.stringify((await cases).map(entry => ({ name: entry.name, replica: a.replica, parent: entry.parent.id,
...
test("all six independently BEAM-judged signed histories reproduce exact link and quarantine projections", async () => {
    ...
    for (const entry of await cases) {
        const expected = fixture.cases.find((item) => item.name === entry.name);
        assert.deepEqual(expected.frames, [...a.frames, entry.parent, entry.child]);
        for (const [frames, projection] of [[expected.frames.filter((frame) => frame.id !== entry.child.id), expected.expected_parent],
            [expected.frames, expected.expected_child]]) {
            const observed = await observe({ replica: a.replica, frames });
            assert.ok(observed.ok, JSON.stringify(observed));
