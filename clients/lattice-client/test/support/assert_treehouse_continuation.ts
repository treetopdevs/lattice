...
import {
  analyzeAuthority, canonicalOrder, carrierOpsToSemanticOps, index, materialize,
  treehouseCommandDecoders, treehouseSpaceSchema, verifyCarrierOp,
} from "../../src/index";
import type { CarrierOpFrame } from "../../src/index";

const expected = JSON.parse(readFileSync(process.argv[2]!, "utf8")) as {
  frames: CarrierOpFrame[]; name: string; threads: unknown[]; adminAction: string;
  holder: string; acquisition: string; reasons: Record<string, string>;
};
for (const frame of expected.frames) {
  const result = await verifyCarrierOp(frame, { verify: async (pub, bytes, sig) =>
