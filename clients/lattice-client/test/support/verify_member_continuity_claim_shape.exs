  ...
  %{
    links:
      Enum.map(observed.links, fn link ->
        %{
          oldPub: Base.encode64(link.old_pub),
          heads: link.heads,
          status: link.status,
          affectedWrappers:
            Enum.map(link.affected_wrappers, &%{opId: &1.op_id, reason: &1.reason})
        }
      end),
...
for item <- fixture["cases"] do
  ...
  for {frames, expected} <- [
        {parent_frames, item["expected_parent"]},
        {item["frames"], item["expected_child"]}
      ],
