...
class BadHelloWebSocket extends ManualCarrierWebSocket {
  static instance: BadHelloWebSocket | null = null;

  static lastInstance(): BadHelloWebSocket | null {
...
function nonceOnlyWebSocket(envelope: unknown) {
  let instance: ManualCarrierWebSocket | null = null;

  return class extends ManualCarrierWebSocket {
    static lastInstance(): ManualCarrierWebSocket | null {
      return instance;
    }
  ...
  assert.deepEqual(socket.sent, [
    {
      ...
      replica: "replica:test",
      nonce: (socket.sent[0] as { nonce: string }).nonce,
...
async function establishSubscription(
  client: CarrierWebSocketClient,
  socket: ManualCarrierWebSocket,
  generation: number,
) {
  const subscribing = client.subscribeAvailability();
  socket.receive({
