...
class ScriptedPageSocket {
  ...
  constructor(private readonly respond: (request: Envelope, index: number) => Envelope) {}

  send(data: string): void {
    const request = JSON.parse(data) as Envelope;
...
const id = (index: number) => String(index).padStart(43, "0");
