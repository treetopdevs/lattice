...
test("missing, forged, duplicated and cross-replica history cannot establish a pin", async () => {
  const v = vectors[0], frames: CarrierOpFrame[] = v.frames;
  for (const bad of [frames.slice(1), [...frames, frames[0]],
    ...
    assert.deepEqual(await resolveContinuationProfileFromFrames({replica: v.replica, frames: bad}),
      {ok: false, reason: "invalid_verified_history"});
  }
  assert.deepEqual(await resolveContinuationProfileFromFrames({replica: v.replica, frames: frames.slice(0, 2)}),
    {ok: false, reason: "continuation_not_configured"});
  assert.deepEqual(await resolveContinuationProfileFromFrames({replica: "legacy", frames: []}),
    {ok: false, reason: "unauthorized_continuation"});
  assert.deepEqual(await resolveContinuationProfileFromFrames({replica: v.replica.replace("-v1", "-v2"), frames: []}),
...
test("latest valid root pin wins while signed impostor and malformed pins remain in the frontier", async () => {
  const v = vectors[0], frames: CarrierOpFrame[] = structuredClone(v.frames);
  async function pin(seedByte: number, profile: unknown, malformed = false) {
    ...
    const term: CarrierTerm = malformed ? ["map", []] : continuationProfileToCarrierTerm(profile)!;
