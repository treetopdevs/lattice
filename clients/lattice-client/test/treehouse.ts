...
import {
  authorTownshipGenesis, authorTreehouseCommand, carrierOpsToSemanticOps,
  materialize, treehouseCommandDecoders, treehouseThreadSchema,
  treehouseSpaceSchema,
...
const signer = { publicKey: ed25519.getPublicKey(privateSeed), sign: (bytes: Uint8Array) => ed25519.sign(bytes, privateSeed) };
...
const aliceSigner = { publicKey: ed25519.getPublicKey(aliceSeed), sign: (bytes: Uint8Array) => ed25519.sign(bytes, aliceSeed) };
...
const roleDecode = (frames: CarrierOpFrame[]) => carrierOpsToSemanticOps(frames, {}, treehouseCommandDecoders("Treehouse.Space"));
...
const decodeSpace = (frames: unknown[]) => carrierOpsToSemanticOps(frames, {}, treehouseCommandDecoders("Treehouse.Space"));
...
] as [string, import("../src/index").CarrierTerm[]][]) {
...
function commandFromOp(op: Op, product: TreehouseProduct): TreehouseCommand {
  const args = op.commandArgs as string[];
  switch (op.command) {
    ...
    case "issue_invitation": return { command: op.command, recipient: args[0]!, threads: op.commandArgs![1] as string[] };
    case "revoke_invitation": return { command: op.command, invitationId: args[0]! };
    case "admit_member": return { command: op.command, invitationId: args[0]!, recipient: args[1]!, level: args[2] as "member" | "moderator", acceptance: args[3]! };
    ...
    case "author_tombstone": case "moderator_tombstone": return { command: op.command, postId: args[0]!, targetId: args[1]! };
...
const reciprocal: Record<string, CarrierOpFrame[]> = {};
...
assert.deepEqual((unicodeResult.state.threads as { replica: string }[]).map((reference) => reference.replica), unicodeScope);
