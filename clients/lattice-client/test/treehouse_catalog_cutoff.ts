...
const derive = (input: Parameters<NonNullable<typeof module>["deriveTreehouseCatalogCutoff"]>[0]) => {
  assert.ok(module); return module.deriveTreehouseCatalogCutoff(input);
};
...
test("rejected evidence and the genuine same-ID op remain separate, ordered and byte exact", async () => {
  const f = await cutoffFixture();
  const rejected = [{ frame: f.forged, reason: "bad_signature" as const }, { frame: f.supplied, reason: "bad_signature" as const }];
  ...
  assert.deepEqual(result.rejected.map((op) => ({ ...op, bytes: Buffer.from(op.bytes).toString("base64"), sig: Buffer.from(op.sig).toString("base64") })),
    [...rejected].sort((a, b) => Buffer.compare(Buffer.from(a.frame.id), Buffer.from(b.frame.id))).map(({ frame }) => ({
  ...
  const pending = derive(mutable); mutable.frames[0]!.sig = ""; mutable.rejected.length = 0;
...
test("accepted corruption, missing closure and malformed rejection claims refuse without repairs", async () => {
  ...
  for (const rejected of [[{ frame: f.genuine, reason: "bad_signature" }], [{ frame: f.forged, reason: "other" }], [{}],
    ...
    assert.deepEqual(await derive({ replica: f.replica, frames: f.frames, rejected } as Parameters<typeof derive>[0]), invalid);
  }
  assert.deepEqual(await derive({ replica: f.replica, frames: [f.genuine], rejected: [{ frame: f.high, reason: "bad_signature" }] }), invalid,
    "rejected evidence cannot close accepted dependencies");
});
...
test("all fixed vocabulary names are shared with the durable fixture and accepted as raw evidence", async () => {
  ...
  const [old, added, names] = fixtures.map(([file, count, hash]) => {
    ...
    const values = JSON.parse(bytes.toString("utf8")) as string[];
    assert.equal(values.length, count); assert.deepEqual([...new Set(values)].sort(), values); return values;
  });
  assert.ok(old && added && names); assert.equal(added.some((name) => old.includes(name)), false);
...
test("broad rejected headers and unrepresentable delegation leases refuse without dropping evidence", async () => {
  ...
  if (wide.body[0] !== "tuple") throw new Error("fixture body");
  const delegation = wide.body[1][1]!;
  assert.equal(delegation[0], "delegation");
  if (delegation[0] !== "delegation") throw new Error("fixture delegation");
...
async function sizedFrame(size: number): Promise<CarrierOpFrame> {
  const f = await cutoffFixture();
...
test("actual single-push envelope allows exactly64000 bytes and refuses64001 without hiding forged input", async () => {
  for (const size of [64_000, 64_001]) {
    ...
    if (size === 64_000) assert.equal(result.ok, true); else assert.deepEqual(result, unsupported);
...
test("generic composite depth64 remains portable and depth65 refuses explicitly", async () => {
  ...
  for (const depth of [64, 65]) {
    let body: CarrierTerm = ["nil"];
    for (let i = 0; i < depth; i++) body = ["list", [body]];
    ...
    if (depth === 64) assert.equal(result.ok, true); else assert.deepEqual(result, unsupported);
  }
});
...
test("cutoff rejects malformed raw scalar tags even when a permissive caller signed their canonical interpretation", async () => {
  ...
  for (const body of [["bool", "true"], ["nil", "extra"]]) {
    const frame = await authorCarrierOp({ replica: f.replica, signer: f.signer, deps: [], kind: "command", cap: ["nil"], body: body as CarrierTerm });
...
test("a representable authenticated kind outside the four wire kinds is unsupported rather than forged", async () => {
  const f = await cutoffFixture();
  const frame = await authorCarrierOp({ replica: f.replica, signer: f.signer, deps: [], kind: "request" as CarrierOpFrame["kind"], cap: ["nil"], body: ["nil"] });
  assert.deepEqual(await derive({ replica: f.replica, frames: [frame], rejected: [] }), unsupported);
  assert.deepEqual(await derive({ replica: f.replica, frames: [{ ...frame, sig: "" }], rejected: [] }), invalid);
});
