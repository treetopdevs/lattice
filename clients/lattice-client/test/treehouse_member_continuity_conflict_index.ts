...
const digest = (label: string) => createHash("sha256").update(`conflict-index:${label}`).digest();
const pub = (label: string) => digest(label).toString("base64");
const id = (label: string) => digest(label).toString("base64url");
function statement(label: string, claim: codec.MemberContinuityClaim): Op {
...
function removalOf(op: Op, key: string) {return op.kind === "command" && op.command === "remove_member" && op.commandArgs?.[0] === key;}
...
function referenceConflicts(
  ...
  const candidates = canonicalOrder([...ops.values()], byId)
    ...
    .filter((entry): entry is {op: Op; certificate: codec.MemberContinuityCertificate} => entry.certificate !== null);
...
test("grouped graph equals pre-index reference for collisions, stale vouchers, duplicate wrappers and inherited refusals", () => {
  for (let seed = 0; seed < 16; seed++) {
    const ops: Op[] = [];
    const claims: codec.MemberContinuityClaim[] = [];
    for (let index = 0; index < 12; index++) {
      ...
      claims.push(claim); ops.push(statement(`mixed:${seed}:${index}`, claim));
    }
    const duplicate = statement(`duplicate:${seed}`, claims[0]!); ops.push(duplicate);
