...
const id = (label: string) => createHash("sha256").update(`member-codec-test:${label}`).digest("base64url");
const key = (label: string) => {
  const seed = createHash("sha256").update(`member-codec-test:${label}`).digest();
  return {pub: Buffer.from(ed25519.getPublicKey(seed)).toString("base64"), sign: (bytes: Uint8Array) => Buffer.from(ed25519.sign(bytes, seed)).toString("base64")};
};
function fixture() {
  ...
  const claim = {version: 1 as const, product: "treehouse" as const,
    space: `replica:treehouse:space:${id("space")}#authority:bounded-continuation-v1#root:${id("root")}`,
    oldPub: old.pub, newPub: next.pub, oldAdmission: id("old-admission"), oldMembership: "active" as const,
...
test("closed member claim preserves exact reviewed key, admission, epoch and ordered voucher evidence", async () => {
  ...
  for (const value of bad) assert.equal(normalizeMemberContinuityClaim(value), null);
...
test("member possession and both vouches bind the exact independent claim and distinct byte purposes", async () => {
  ...
  const certificate = {claim, possession, vouches: vouchers.map((member) =>
    ({member: member.pub, signature: member.sign(codec.canonicalBytesForMemberContinuityVouch(claim, possession))}))};
  ...
  const bin = (value: string): ["bin", string] => ["bin", Buffer.from(value).toString("base64")];
  ...
  ] as const) assert.deepEqual(payload, canonical.canonicalBytesForCarrierTerm(["list", [bin(domain), term]]));
  ...
  for (const value of bad) assert.equal(codec.verifyMemberContinuityCertificate(value, claim), false);
...
test("carrier adapters preserve closed maps and reject duplicate keys, tags and malformed text before signature use", async () => {
  ...
  if (reversed[0] === "map") reversed[1].reverse();
  ...
  if (rawClaim[0] !== "map") return;
  const duplicate = structuredClone(rawClaim); duplicate[1][1] = structuredClone(duplicate[1][0]!);
  ...
  for (const [field, value] of [["space", ["bin", "/w=="]], ["product", ["bin", Buffer.from("treehouse").toString("base64")]],
    ...
    ["epoch", ["int", "00"]], ["deps", ["tuple", []]]] as const) {
    const bad = structuredClone(rawClaim) as ["map", [unknown, unknown][]];
  ...
  const challenge = {version: 1 as const, product: "treehouse" as const, space: claim.space, oldPub: claim.oldPub,
...
test("the narrow decoded-argument adapter consumes real signed carrier decode and refuses metadata sentinels", async () => {
  ...
  let decoded: import("../src/carrier").DecodedTerm[] = [];
  const decoders: import("../src/carrier").CommandDecoderMap = new Map([["attest_member_key_v1", {arity: 3, decode: (values) => {
    decoded = values; captured = codec.memberContinuityCertificateFromDecodedArguments(values);
  ...
  for (const position of [0, 1, 2]) {
    const invalid = structuredClone(decoded); invalid[position] = {type: "invalid_beacon_integer"};
  ...
  if (bad[0]?.type === "map") bad[0].pairs[1] = structuredClone(bad[0].pairs[0]!);
...
test("detached return accepts the largest v1 challenge below64000 and refuses its next genuinely signed size", async () => {
  ...
  for (const heads of [1, 16]) {
    const challenge = {version: 1 as const, product: "treehouse" as const, space: claim.space, oldPub: old.pub,
...
test("nested voucher and signature maps remain closed and byte order is unsigned key order", async () => {
  ...
  const term = codec.memberContinuityCertificateToCarrierTerm(certificate)!;
  for (const target of ["claim-voucher", "vouch"] as const) {
    const copy = structuredClone(term) as any;
    const fields = new Map(copy[1].map(([key, value]: any) => [key[1], value]));
    const entries = target === "vouch" ? fields.get("vouches") as any :
      new Map((fields.get("claim") as any)[1].map(([key, value]: any) => [key[1], value])).get("vouchers") as any;
...
for (const producer of ["beam", "ts"] as const) test(`${producer} independently signed fixture verifies all four purposes and remains ungranted under its historical ceiling`, async () => {
