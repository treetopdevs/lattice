...
import type { WitnessedSuccessionClaimEvidence } from "../src/index";
...
assert.equal(
  exportWitnessedSuccessionArtifactJson(artifact),
  `{"v":1,"artifactId":"${artifactId}","claim":{"version":1,"replica":"${claim.replica}","role":"clerk","holder":"${claim.holder}","holderEpoch":"${claim.holderEpoch}","successor":"${claim.successor}","policyId":"${claim.policyId}"},"witness":"${witness}","signature":"${signature}"}`,
);
...
assert.equal(
  exportWitnessedSuccessionArtifactJson(
    assembleWitnessedSuccessionArtifact(reorderedClaim, { signature, witness }),
  ),
  exportWitnessedSuccessionArtifactJson(artifact),
);
...
for (const malformed of [
  ...
  assert.throws(() =>
    assembleWitnessedSuccessionArtifact(
      malformed as WitnessedSuccessionClaimEvidence,
      { witness, signature },
    ),
  );
}
