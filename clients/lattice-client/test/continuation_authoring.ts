...
test("valid high legacy epochs block bounded continuation while unauthorized high input stays inert", async () => {
  for (const kind of ["space", "thread"] as const) {
    for (const high of ["9007199254740992", "18446744073709551615"]) {
      for (const authorized of [true, false]) {
        ...
        if (authorized) assert.deepEqual(result, {ok: false, reason: "invalid_continuation_epoch"});
        else {
          assert.equal(result.ok, true);
          if (result.ok) assert.equal(result.review.claim.epoch, 5);
        }
      }
    }
  }
