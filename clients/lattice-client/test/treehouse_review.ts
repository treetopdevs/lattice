...
import {
  analyzeAuthority, authorTownshipGenesis, authorTreehouseCommand, canonicalOrder,
  carrierDelegationsFromFrames, carrierOpsToSemanticOps, index, materialize,
  treehouseCommandDecoders, treehouseSpaceSchema, verifyCarrierOp,
} from "../src/index";
...
const decode = (frames: CarrierOpFrame[]) => carrierOpsToSemanticOps(frames, {}, treehouseCommandDecoders("Treehouse.Space"));

if (process.env.TREEHOUSE_REVIEW_CASE !== "map") {
  const duplicate = structuredClone(genesis);
  if (duplicate.body[0] !== "tuple" || duplicate.body[1][1]?.[0] !== "delegation") throw new Error("genesis shape");
  duplicate.body[1][1][1].roles = ["admin", "admin", "moderator"];
  const verified = await verifyCarrierOp(duplicate, { verify: async (pub, bytes, sig) =>
    ed25519.verify(sig, bytes, Buffer.from(pub, "base64"), { zip215: false }) });
...
if (process.env.TREEHOUSE_REVIEW_CASE !== "roles") {
  ...
  for (const title of titles) frames.push(await authorTreehouseCommand({ product: "Treehouse.Space", replica: genesis.replica,
  ...
  for (const delivery of [frames, [...frames].reverse()]) {
    ...
    assert.deepEqual((view.state.threads as { title: string }[]).map((entry) => entry.title), expected);
