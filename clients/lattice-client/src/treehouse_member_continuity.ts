...
import {
  ...
  memberContinuityClaimFromCarrierTerm,
...
export function memberContinuityCommandConflicts(
  ...
  const byNewPub = new Map<string, typeof candidates>();
  const parentWrappers = new Map<string, Map<string, typeof candidates>>();
  ...
  for (const candidate of candidates) {
    ...
    targets.push(candidate); byNewPub.set(claim.newPub, targets);
    if (referencedParents.size > 0) {
      const claimId = memberContinuityClaimId(claim)!;
      if (referencedParents.has(claimId)) {
        const byOldPub = parentWrappers.get(claimId) ?? new Map<string, typeof candidates>();
        const wrappers = byOldPub.get(claim.oldPub) ?? [];
        wrappers.push(candidate); byOldPub.set(claim.oldPub, wrappers); parentWrappers.set(claimId, byOldPub);
      }
    }
  }
  const removalsByRecipient = new Map<string, Op[]>();
  for (const op of ops.values()) {
    if (verdicts.get(op.id) !== "honored" || op.kind !== "command" || op.command !== "remove_member") continue;
    const recipient = op.commandArgs?.[0];
    if (typeof recipient !== "string") continue;
    const removals = removalsByRecipient.get(recipient) ?? [];
    removals.push(op); removalsByRecipient.set(recipient, removals);
  }
  const denied = new Map<string, string>();
  for (const candidate of candidates) {
    const claim = candidate.certificate.claim;
    const stale = claim.vouchers.some((voucher) => (removalsByRecipient.get(voucher.member) ?? []).some((removal) =>
      concurrent(candidate.op.id, removal.id, byId, ancestorCache) &&
    ...
    else if ((byNewPub.get(claim.newPub) ?? []).some((other) =>
      claim.oldPub !== other.certificate.claim.oldPub && concurrent(candidate.op.id, other.op.id, byId, ancestorCache))) {
      denied.set(candidate.op.id, "application_continuity_conflicting_target");
    }
  }
  for (const candidate of candidates) {
    if (denied.has(candidate.op.id)) continue;
    const claim = candidate.certificate.claim;
    const invalid = claim.parents.some((parent) => !(parentWrappers.get(parent)?.get(claim.oldPub) ?? []).some((wrapper) =>
      ancestors(candidate.op.id, byId, ancestorCache).has(wrapper.op.id) && !denied.has(wrapper.op.id)));
    if (invalid) denied.set(candidate.op.id, "application_continuity_invalid_parent");
  }
  return denied;
}
...
export async function assembleMemberContinuityFromFrames(input: {
  ...
  try {
    ...
    } catch { return refuse("signer_failed"); }
    try {
      if (!(await verifyCarrierOp(frame, {verify: async (author, bytes, signature) =>
        ed25519.verify(signature, bytes, base64ToBytes(author), {zip215: false})})).valid) {
        return refuse("invalid_signer_signature");
      }
    } catch { return refuse("invalid_signer_signature"); }
...
function claimFromFrame(frame: CarrierOpFrame): MemberContinuityClaim | null {
  const body = frame.body;
  if (body[0] !== "tuple" || body[1].length !== 2 || body[1][0]?.[0] !== "atom" ||
    body[1][0][1] !== "attest_member_key_v1" || body[1][1]?.[0] !== "list" || body[1][1][1].length !== 3) return null;
