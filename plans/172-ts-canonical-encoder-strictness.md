...
**Adopted R07 amendment (2026-09-06):** The unified Treehouse plan authorizes the
strict-refusal implementation and the deliberate fixture-contract amendment below.
The RED tests at `a6d685b83f45589e00624cf14c80329953220f40` were replayed against
`641cbbd78bf1338a4a245e5a670ad425aa79be1b`; all ten negative checks still failed.
`codec.ts` and the pre-RED `canonical.ts` match the prior `9601f146` baseline exactly.
The only newer `authority.ts` change is the independent foreign-replica judgment near
line 147; its decoder call sites are unchanged and that judgment is preserved.
The public codec, verification, and carrier-decoding seams in the test plan are approved.
Claude Fable's consumer review extended this amendment to the public root-binding and
native-signer seams: `township.ts` and `tauri_bridge.ts` must use the shared strict
decoder for imported root keys and native public-key/signature replies. Keys are 32
bytes and signatures are 64 bytes; native typed byte arrays also retain those bounds.
The carrier decoder at `carrier.ts:2092` already checks the standard alphabet/padding
and exact round-trip before returning bytes, so its guarded conversion is retained.

**In scope**:

- `clients/lattice-client/src/codec.ts` — `encodeMap`, the `"mapset"` arm, and the base64
  decoder consolidation
- `clients/lattice-client/src/authority.ts` — consolidate the duplicate Base64 decoder
  and canonicality helper, import the shared decoder, and route its two former lenient
  consumers (`delegationSelfConsistent` and `canonicalBase64UrlDigest`) through it.
  Preserve their `false` failure contracts and all authority policy/replica logic.
- `clients/lattice-client/test/canonical.ts` — negative cases
- `clients/lattice-client/src/township.ts` and `src/tauri_bridge.ts` — only the
  consumer decoder consolidation and known key/signature lengths described above
- `clients/lattice-client/test/township_authoring.ts` and `test/tauri_bridge.ts` —
  public root-binding/native-signer refusals and their existing positive controls
- `clients/lattice-client/test/carrier.ts` — the injected binary-whitespace fixture
  must expect strict canonical hashing to refuse `non-canonical base64`, alongside the
  existing frame decoder's `malformed carrier op`. The fixture remains malformed;
  production validation must not be loosened and generated vectors must not change.
- `plans/172-ts-canonical-encoder-strictness.md` — this adopted amendment and evidence
- `clients/lattice-client/dist/**` — regenerated by `npm run build`, **never hand-edited**
- `plans/README.md` — status row, updated by the unified-plan integrator
...
- `clients/lattice-client/src/authority.ts` beyond that decoder consolidation — **plans 162 and 163 own
...
step 1 now pass; `rg -F 'Buffer.from(value, "base64")' src/authority.ts src/codec.ts src/township.ts src/tauri_bridge.ts`
shows exactly one occurrence remaining (inside the shared decoder). The count covers
the four scoped files; the already strict carrier decoder retains its guarded conversion.
...
- Any carrier or conformance suite goes red after step 4 because a **fixture** carries
  non-canonical base64, except the injected binary-whitespace fixture explicitly
  amended above. Report any other fixture; do not loosen the decoder to accommodate it.
...
### R07 implementation evidence, 2026-09-06

Implementation worktree: `lattice-treehouse-r07-20260906`, based on unified proposal
`641cbbd78bf1338a4a245e5a670ad425aa79be1b`. RED commit `f898f414` replays the
identified prior test patch; the prior dirty worktree remains untouched. The original
ten negatives failed, the duplicate-only implementation left eight failures, and the
strict decoder closed all ten. Subsequent root-binding and native-signer tests produced
four and ten failures before their consumer fixes. A separate browser-path regression
exposed a 256 KiB valid binary rejected by a whole-array spread; bounded encoding chunks
made it pass without changing canonical bytes.

Verification passed:

- TypeScript typecheck, 964 conformance checks, 180 canonical checks, 52 Township
  authoring checks, 21 Tauri bridge checks, and 49 carrier checks.
- Relay, relay-sync and feed suites; 68 live TS/BEAM carrier checks; client build;
  shell typecheck and native workflow contract; mobile product-manifest contract.
- `mix verify`: formatting plus every default umbrella suite, 663 reported tests,
  27 properties, zero failures, and three configured exclusions. This is default
  local verification, not packaged-device or release evidence.
- Vector export completed with an empty diff against the proposal baseline. Every
  pre-existing vector, including Plan 179's named legacy vectors, and
  `Lattice.Canonical` are unchanged. All corpus Base64 values were canonical.
- Exactly one raw Base64 conversion remains across the four amended source files;
  the separate carrier conversion remains protected by its existing strict checks.

The build also refreshed two existing source assertions in `dist/test/conformance.js`
(`capabilityBeforeApplication` and `wrongKind`) that were missing from the baseline's
generated artifact. No conformance source or oracle expectation changed. The shared
README/unified execution status remains the integrator's responsibility.
