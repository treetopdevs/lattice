...
| 162 | Bind root-less delegations and genesis authorship to the replica root | **P0** | M | 161 (rec.) | DONE (Round 5 amendment step 2b executed — `cap_ok/9` + `validate_delegation/6` cross-replica guard and `role_event/3` malformed-tick guard in Elixir, compaction_spike, `authority.ts`, and `capability.ts`; unbound validate skip dropped to match `cap_ok`/`verify_chain`; same-root sibling genesis/transfer/succeed additionally refused at timeline time before `record_acquire/4`; mutation evidence corrected; vectors regenerated, all gates green) |
