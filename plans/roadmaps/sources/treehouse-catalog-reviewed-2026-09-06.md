...
Prepared 2026-09-06 for unified R11a/R11b/R11c. **Proposed for exact review;
no production implementation or profile enablement is authorized by this file
alone.** The integrator accepted preparation of the delegated-admin approach;
the closed records, transitions and source-contract amendments below still need
review before implementation. R04 is frozen separately; this design worktree
...
hosted-review remediation is still open, so adoption and implementation must
recheck against its final landed judge before any R11 enablement.
...
## 7. Exact source-contract amendments proposed before code

Append a dated R11 amendment to Plan158's catalog section, preserving historical
...
Append to Plan178 immediately after its Space command vocabulary, explicitly
extending the ordered list by these two entries, before implementation:
...
Before code the integrator/reviewer must accept or revise the exact maps/domains,
...
fork/reclassification resolution rules. These are concrete proposed defaults,
not requests to invent constants after implementation. A needed Core semantic
