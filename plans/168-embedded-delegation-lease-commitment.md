...
The live corpus had drifted since the plan was written: five generated scenarios already carried
leased delegations. Their expected op ids were regenerated, and the exporter now includes BEAM
canonical-byte oracles for those leased frames so TypeScript parity is checked byte-for-byte.
The new hashes also exposed locale-dependent ordering in
`clients/lattice-client/test/conformance.ts`; that harness and its command-table producer now
use explicit codepoint ordering. This is a required scope extension, not an authority-semantic
change.

Adversarial review also disproved step 5's predicted `FunctionClauseError`: the generic struct
encoder converts malformed delegation leases to `ArgumentError`. Execution therefore keeps the
narrow `ArgumentError` rescue and adds an explicit malformed-lease clause rather than broadening
the rescue to hide unrelated encoder defects.
...
- `apps/lattice_core/lib/lattice/canonical.ex` — the `%Delegation{}` `encode/1` clauses, including
  the explicit malformed-lease clause that preserves the narrow `signable?/1` rescue
- `clients/lattice-client/src/codec.ts` — `encodeDelegation` plus sibling delegation-payload
  validation parity discovered during adversarial review
- `clients/lattice-client/src/carrier.ts` — codepoint ordering for the command-table producer,
  required by the conformance-harness normalization exposed by regenerated ids
...
- `clients/lattice-client/test/conformance.ts` — replace locale-dependent comparison with explicit
  codepoint ordering after regenerated ids exposed the mismatch
...
- `plans/168-embedded-delegation-lease-commitment.md` — execution reconciliation and scope record
