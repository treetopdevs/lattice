defmodule Township.WorkflowsTest do
  ...
  test "W0 — join by vouch: only a member-issued Cap admits a new identity" do
    ...
    # (Represented here by the gateway/grant scoping: resident was granted :admit,
    # an un-granted realm would be denied. The negative is exercised in full in
    # the capability suite carried forward from V1.)
  end
  ...
  test "W1 — durable deliberation: partition + heal converges the summary" do
    ...
    {sim, _} = Sim.create_replica(sim, "clerk",
      policies: %{clerk: %{successor: "resident", dormant_ticks: 3}})
    {sim, _} = Sim.grant(sim, "clerk", "resident",
      ops: [:post, :set_summary, :set_title])
    sim = Sim.sync_all(sim)
  ...
  test "W2 — roles & consent: stale post-transfer clerk op is quarantined" do
    ...
    {sim, _} = Sim.create_replica(sim, "clerk_a",
    ...
    # Transfer :clerk authority to clerk_b.
    {sim, _} = Sim.transfer(sim, "clerk_a", "clerk_b", :clerk, at_tick: 1)
    sim = Sim.sync_all(sim)

    # clerk_a, unaware, emits another clerk-only op AFTER the transfer.
    ...
    assert {true, _reason} = Sim.quarantined(sim, "clerk_a", stale_op.id)
    assert {true, _reason} = Sim.quarantined(sim, "clerk_b", stale_op.id)
