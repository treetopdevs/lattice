defmodule Lattice2.ConvergencePropertyTest do
  @moduledoc """
  The required StreamData property suite (design: property-based testing).

  Generators produce three realms, randomized command sequences (convergent +
  authoritative + transfers) and randomized partition/heal/sync schedules, seeded.
  Properties checked after a full heal + sync:

    a. all realms converge to equal state;
    b. the authoritative field (`locked?`) was only ever mutated by the holder at
       each honored op's causal position (single-writer authority);
  ...
  defp run(actions, seed) do
    ...
    {sim, _} = Sim.grant(sim, "r0", "r1", ops: [:post, :set_title, :join, :leave])
    {sim, _} = Sim.grant(sim, "r0", "r2", ops: [:post, :set_title, :join, :leave])
  ...
  defp apply_action({:lock, i}, {sim, t}), do: {elem(Sim.command(sim, realm(i), :lock, []), 0), t}
  ...
  property "b: locked? is only ever mutated by the holder at each honored op's causal position" do
    check all(
      ...
      sim = run(actions, seed)
      log = Sim.log(sim, "r0")
      quarantine = Authority.quarantine(Thread, log)
      ops = Log.ops(log)
      ...
      honored_locks =
        ...
        |> Map.values()
        |> Enum.filter(fn op ->
          op.kind == :command and elem_first(op.body) in [:lock, :unlock] and
