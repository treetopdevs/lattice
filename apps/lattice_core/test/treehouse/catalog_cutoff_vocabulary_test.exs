defmodule Treehouse.CatalogCutoffVocabularyTest do
  ...
  @artifacts [
    {"cutoff_atoms_v1.json", 130,
     "a66d085dd185091d745d933c3145101a97b3306406aba905af07ca051d09506c"},
    {"cutoff_atoms_member_continuity_v1.json", 9,
     "38aab5c25051af1d009c917274cdf68ae4f61776ebef0bf3634ad3899b9ce8df"},
    {"cutoff_atoms_with_member_continuity_v1.json", 139,
  ...
  test "historical and additive hashes bind the exact three compiled vocabularies" do
    [old, added, union] =
      for {file, count, hash} <- @artifacts do
  ...
  test "signed nine and 139-name histories are portable while remaining ordinarily refused" do
    ...
    for {file, _, _} <- Enum.drop(@artifacts, 1) do
      ...
      for delivered <- [
            ...
            Log.topo_ops(fixture.space_log) ++ [op]
          ] do
        assert {:quarantined, rejected, :bad_signature} =
                 Log.accept(Log.new(op.replica), %{op | sig: <<0::512>>})
  ...
  test "raw trust installation and reopen retain new-name signed and rejected evidence" do
    ...
    atoms =
      @client
      |> Path.join("test/vectors/catalog/cutoff_atoms_member_continuity_v1.json")
      |> File.read!()
      |> Jason.decode!()
      |> Enum.map(&String.to_existing_atom/1)
    ...
    for frames <- [raw.frames, Enum.reverse(raw.frames)] do
      ...
      assert %{kind: :propose, next: installed, routes: routes} =
               evaluate(prepared, [fixture.catalog_json], histories)
      ...
      assert Enum.any?(
               retained.rejected,
               &(&1.frame["id"] == op.id and &1.frame["sig"] == Base.encode64(<<0::512>>))
             )
    ...
    for atom <- [:cutoff_unknown_evidence_v99, :claim_id] do
      ...
      assert %{kind: :reject, reason: :unsupported_cutoff} =
               CatalogTrust.prepare_installation(%{
                 review: fixture.review,
                 history: bad,
                 store: %{kind: :verified_fresh, expected: @expected}
    ...
    forged = %{
      raw
      | frames:
          Enum.map(raw.frames, fn frame ->
            if frame["id"] == op.id,
              do: Map.put(frame, "sig", Base.encode64(<<0::512>>)),
              else: frame
          end)
    }
  ...
  test "raw trust refuses duplicate containers and alternate uint64 spelling without changing Wire" do
    fixture = CatalogTrustVectors.fixture(0)

    for body <- [%{new_pub: <<>>}, MapSet.new([:active]), 18_446_744_073_709_551_615] do
      op =
        Op.new(
          fixture.space.root,
          fixture.space.replica,
      ...
      raw = fixture.space_log |> Log.append!(op) |> CatalogTrustVectors.raw_history()
      ...
      malformed =
        Enum.map(raw.frames, fn frame ->
          if frame["id"] == op.id do
            body =
              case frame["body"] do
                ["map", [pair]] -> ["map", [pair, pair]]
                ["mapset", [value]] -> ["mapset", [value, value]]
                ["int", value] -> ["int", "0" <> value]
              end

            Map.put(frame, "body", body)
          else
            frame
          end
        end)
      ...
      assert %{kind: :reject, reason: :unsupported_cutoff} =
               CatalogTrust.prepare_installation(%{
                 review: fixture.review,
                 history: %{raw | frames: malformed},
                 store: %{kind: :verified_fresh, expected: @expected}
               })
    end
  end

  defp evaluate(installed, catalogs, histories),
    do:
