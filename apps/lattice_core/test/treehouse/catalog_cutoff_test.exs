defmodule Treehouse.CatalogCutoffTest do
  ...
  test "cutoff binds exact authenticated operations including semantic quarantine" do
    ...
    expected_ops =
      f.log |> Log.ops() |> Map.values() |> Enum.sort_by(& &1.id)
      |> Enum.map(&%{id: &1.id, bytes: Canonical.op_payload(&1), sig: &1.sig})

    bytes = Canonical.term(["lattice-treehouse-recovery-cutoff-v1", f.log.replica, expected_ops, []])
    assert result.ops == expected_ops
    assert result.rejected == []
    assert result.canonical_bytes == bytes
    assert result.cutoff == %{replica: f.log.replica, frontier: Log.frontier(f.log),
  ...
  test "rejected signature evidence survives alongside a subsequently accepted genuine op" do
    f = history()
    genuine = Op.new(f.root, f.log.replica, Log.frontier(f.log), :command,
      {:create_space, ["Retained"]}, cap: f.cap)
    ...
    assert result.rejected == [%{id: forged.id, bytes: Canonical.op_payload(forged),
      sig: forged.sig, reason: :bad_signature}]
  ...
  test "incomplete or forged accepted history refuses without repairing the source" do
    f = history()
    for bad <- [%{f.log | ops: Map.delete(f.log.ops, f.genesis.id)},
      %{f.log | ops: Map.put(f.log.ops, f.denied.id, %{f.denied | sig: <<0::512>>})},
      %{f.log | referenced: MapSet.new()}, %{f.log | quarantine: [%{reason: :bad_signature}]}] do
  ...
  defp history do
    ...
    denied = Op.new(root, log.replica, Log.frontier(log), :command,
      {:create_thread, [[], "Invalid"]}, cap: delegation.id)
    %{log: Log.append!(log, denied), genesis: genesis, root: root,
      denied: denied, cap: delegation.id}
  end
end
