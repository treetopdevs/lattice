defmodule Treehouse.ContinuationProfileObservationTest do
  ...
  test "only the latest valid root pin is selected; impostors and malformed replacements stay retained" do
    {sim, _} = F.new()
    assert {:error, :continuation_not_configured} = Authority.continuation_profile(Sim.log(sim, "founder"))
    ...
    assert Enum.all?([impostor, first, second, malformed], &MapSet.member?(Log.op_ids(log), &1.id))
  end
  ...
  test "forged or incomplete retained history never produces a trusted pin" do
    ...
    for bad <- [Log.from_ops(sim.replica, Map.delete(ops, genesis.id)),
                 Log.from_ops(sim.replica, Map.put(ops, pin.id, forged)),
                 %{log | referenced: MapSet.new()}] do
  ...
  test "legacy and unknown intended families have distinct closed refusals" do
    ...
    assert {:error, :unauthorized_continuation} = Authority.continuation_profile(Sim.log(legacy, "founder"))
    {bounded, _} = F.new()
    log = Sim.log(bounded, "founder")
    unknown = %{log | replica: String.replace(log.replica, "bounded-continuation-v1", "bounded-continuation-v2")}
