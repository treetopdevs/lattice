defmodule Treehouse.MemberContinuityVocabularyTest do
  ...
  @vectors Path.expand("../../../../clients/lattice-client/test/vectors/member_continuity", __DIR__)

  test "selected Space alone interns precisely the adopted application supplement" do
    names = Path.expand("../../../../clients/lattice-client/test/vectors/catalog/cutoff_atoms_member_continuity_v1.json", __DIR__)
    script = prelude() <> """
    names = System.argv() |> hd() |> File.read!() |> Jason.decode!()
    true = Enum.map(Treehouse.Space.known_continuity_wire_atoms(), &Atom.to_string/1) == names
  ...
  test "first real signed frame decodes after only selected Space loading and remains unknown" do
    ...
    """
    assert run!(script, Enum.map(["beam_codec.json", "ts_codec.json"], &Path.join(@vectors, &1))) =~ "SELECTED_SPACE_FIRST_FRAME_OK"
  end

  @tag :tmp_dir
  test "separate cold verified restore retains authentic bytes and refuses forged or unsafe dumps", %{tmp_dir: dir} do
    ...
    assert :ok = Log.dump(%{source.log | ops: Map.put(source.log.ops, source.op.id, %{source.op | sig: <<0::512>>})}, forged)
    unknown = Op.new(fixture.root, source.op.replica, [source.op.id], :command, :member_continuity_unknown_dump_v99)
    ...
    """
    assert run!(script, [genuine, forged, unsafe, Path.join(@vectors, "beam_codec.json")]) =~ "SELECTED_SPACE_VERIFIED_RESTORE_OK"
  end
  ...
  defp run!(script, args) do
    {output, status} = System.cmd(System.find_executable("elixir"),
      ["-pa", Application.app_dir(:lattice_core, "ebin"), "-pa", Application.app_dir(:jason, "ebin"), "-e", script | args],
