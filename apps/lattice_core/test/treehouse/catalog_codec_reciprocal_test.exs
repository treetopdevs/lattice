defmodule Treehouse.CatalogCodecReciprocalTest do
  ...
  test "TypeScript's independently signed planned rotation verifies both fixed purposes in BEAM" do
    ...
    assert Enum.sort(Map.keys(rotation.rotation)) ==
             Enum.sort(
               @rotation_fields --
                 [:rotation, :old_signature, :new_signature, :replica, :frontier, :log_digest]
             )

    assert :ok =
             TransportCatalog.verify_rotation(
               rotation,
               Base.decode64!(vector["expected_old_key"])
             )

    assert Base.encode64(TransportCatalog.rotation_bytes(rotation.rotation)) ==
             vector["rotation_bytes"]

    assert Base.encode64(TransportCatalog.rotation_possession_bytes(rotation.rotation)) ==
             vector["possession_bytes"]
  ...
  test "changed transition fields or purpose signatures cannot reuse the legitimate rotation" do
    ...
    for changed <- [
      ...
      assert {:error, :invalid_rotation_signature} =
               TransportCatalog.verify_rotation(changed, key)
    end

    for malformed <- [
          Map.put(envelope, :extra, 1),
          %{envelope | rotation: %{envelope.rotation | cutoffs: []}},
          %{envelope | rotation: %{envelope.rotation | generation: 0}},
          %{envelope | rotation: Map.put(envelope.rotation, :service_key, key)}
        ] do
  ...
  test "standalone public catalog bytes and inventory IDs match the independent TS signer" do
    ...
    assert :ok =
             TransportCatalog.verify_catalog(
               envelope,
               Base.decode64!(vector["expected_catalog_key"])
             )

    assert Base.encode64(TransportCatalog.catalog_bytes(envelope.catalog)) ==
             vector["catalog_bytes"]

    assert TransportCatalog.catalog_id(envelope.catalog) == vector["catalog_id"]

    assert Base.encode64(TransportCatalog.inventory_bytes(envelope.catalog.entries)) ==
             vector["inventory_bytes"]
    ...
    assert TransportCatalog.service_realm(bootstrap.service_id) ==
             "treehouse-service:" <> bootstrap.service_id
  end

  defp fixture do
    Path.expand(
      "../../../../clients/lattice-client/test/vectors/treehouse_catalog/ts_codec.json",
      __DIR__
    )
    |> File.read!()
    |> Jason.decode!()
