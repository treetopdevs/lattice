defmodule Lattice.Authority.Delegation do
  ...
  defstruct [:id, :replica, :issuer, :audience, :parent_id, :ops, :roles, :live, :sig]
  ...
  def new(%Identity{} = issuer_identity, replica, audience_pub, opts \\ []) do
    ...
    encoding = encode(replica, issuer_identity.pub, audience_pub, parent_id, ops, roles, live)
  ...
  def valid_sig?(%__MODULE__{} = d) do
    encoding = encode(d.replica, d.issuer, d.audience, d.parent_id, d.ops, d.roles, d.live)
    d.id == hash(encoding) and Identity.verify(d.issuer, encoding, d.sig)
  end

  @doc "True if `child` legally attenuates `parent` (subset capability, linked issuer)."
