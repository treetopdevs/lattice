...
Plan 117 adds Android release visible chooser onboarding: the release smoke installs a distinct
diagnostic `township://` handler, observes both `Township` and `Township Diagnostic` in the Android
`uiautomator` resolver hierarchy, taps the primary `Township` row, and then drives the same
runtime-state onboarding pull-author-reload-push-report flow to convergence.
Plan 118 adds desktop Tauri default-namespace onboarding convergence through `onboarding:contract`:
the desktop orchestration imports a pairing handoff, saves peer config under the default
`TOWNSHIP_STORAGE_NAMESPACE`, runs initial sync to pull the cap, authors a post through the native
signer seam, runs final sync, observes a drained outbox, and asserts no resident private seed
material is present in app KV.
Plan 119 adds packaged macOS app-runtime onboarding convergence through
`tauri:onboarding:smoke`: the dev-trace release-mode `.app` runs that same public ceremony against a
live BEAM peer, authors the existing Sim-exported post through the native signer, drains the default
outbox, and writes only replayable state to an isolated native KV file. This does not prove human
click-through of the pairing, sync, or post controls.
This still does not prove Chrome/browser navigation in general, cross-device pairing state exchange,
...
runtime-state onboarding pull-author-reload-push-report flow to convergence.
Plan 118 adds desktop Tauri default-namespace onboarding convergence through `onboarding:contract`:
the desktop orchestration uses the default `TOWNSHIP_STORAGE_NAMESPACE` to prove imported pairing
handoff save, initial sync, post authoring, final sync, drained outbox, and no resident private
seed material in app KV. It does not prove QR camera onboarding, does not prove LAN discovery, does
not prove physical-device behavior, and does not prove iOS, Expo, cross-device pairing state
exchange, or full mobile onboarding.
Plan 119 adds packaged macOS app-runtime onboarding convergence through
`tauri:onboarding:smoke`: the dev-trace release-mode app imports the public handoff, pulls the
Sim-derived cap/prefix from a live BEAM peer, signs and pushes the Sim-exported post, drains the
default outbox, and proves no resident private seed material enters its isolated native KV file. It
does not prove human click-through or any mobile, cross-device, QR/LAN, or production TLS claim.
Plan 129 reuses this custody strategy unchanged for a second packaged macOS gate against the stable
durable relay. The public pairing records explicit relay mode, the existing native key signs the
carrier sessions and exact Sim-generated post, ordinary app KV retains replayable cap/log state,
and only durably acknowledged frames leave the outbox. A fresh pull-only BEAM observer matches Sim
after server restart, while private seed material remains absent from KV and traces. This does not
add a mobile secure-store implementation, mobile relay proof, iOS/Expo result, or phone-grade claim.
Plan 130 reuses this custody strategy unchanged for the first LiveView-to-app participant post
handoff. Deep-link ingress stages only a bounded unsigned request; the app validates its replica
against public pairing state, and the existing native signer plus ordinary replayable KV state run
only after explicit Use request, Post, and Sync actions. The packaged macOS gate proves this desktop
custody path through LaunchServices, but adds no mobile secure-store implementation, mobile relay,
iOS/Expo, physical-device, or phone-grade claim.
Plan 131 reuses this custody strategy unchanged while making the two packaged macOS proofs mandatory
