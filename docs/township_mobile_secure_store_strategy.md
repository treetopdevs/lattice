...
2. The app-storage implementation reloads `local_ops`, `carrier_frames`, and `delegation_frames`
   ...
   - Packaged macOS Tauri app: real-app armed one-shot OS delivery is met by plan 096 in an explicit `township-dev-trace` release-mode smoke build; the traced no-side-effect link-load guard is met by plan 097; warm LaunchServices scheme resolution is met by plan 098; cold-start URL delivery is met by plan 099; app-local state binding is met by plan 100; release dev-trace hydration and control-link repair is met by plan 101; default-namespace desktop onboarding is met at the TypeScript orchestration seam by plan 118 and in the packaged app runtime against a live BEAM peer by plan 119; packaged stable-relay native-custody convergence and restart recovery are met by plan 129. Plan 119 does not prove human click-through, and Plan 129 does not add a mobile claim.
   - iOS Tauri and Expo: still unproven.
