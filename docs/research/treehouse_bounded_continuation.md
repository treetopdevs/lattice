...
Initial dependency integration passed exact Fable review; the final contextual
metadata follow-up is locally verified and awaits its bounded review and hosted
integration. No product profile
...
### Complete reserved-metadata follow-up

The initial dependency integration at `c2d85b92` and evidence tip `24623546`
passed the final Claude Fable integration review with no actionable finding.
Integration `a875e1adb46324ad6350fd12fdfadc067eb3682c` adds R03's complete
contextual metadata repair `a29c4510707ecb9a4e125ab4bd94cbc642affb57`, which
separately passed actual Fable completion review. Source merges automatically;
the sole generated authoring-test conflict was regenerated by the normal TS
build. R04's raw body/replica parsing, inner refusal adapter and all three
resolved-root collectors remain unchanged. No R10 product implementation is added.

At a875e1ad, full `mix check` again passes **772 tests + 27 properties, zero
failures**, three existing exclusions, clean formatting and strict Credo exit 0.
TS typecheck, build and conformance pass, including the complete reserved-policy
and certificate-metadata controls. Logs are
