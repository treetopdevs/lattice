...
test("stable carrier restart patches fresh to stale to fresh without transferring participant custody", async ({ page }) => {
  ...
  await expect(page.locator("#participant-title-handoff")).toHaveCount(0);
  await expect(page.locator("#participant-summary-handoff")).toHaveCount(0);
  ...
  await expect(page.locator("#participant-roster-handoff")).toHaveCount(0);
