...
5. **G1 (physical BEAM carrier) is now reachable outside `Sim`** — plan 017 runs W0–W3
   ...
   in `plans/README.md`**; coverage and enforcement through plans 023-137 carry their own gates and
   non-claims, so read
...
Plan 135 closes the first clerk-status slice, Plan 136 closes the field-edit slice, and Plan 137
closes the roster slice toward **complete G1/Phase G**. Capability-lifecycle controls, production
deployment, and receipt-free W4 are not supplied by the current work. At the milestone level,
broader participant controls, production deployment, and receipt-free W4 remain.
Broader participant controls remain as the broad label for the unfinished capability-lifecycle slice.
The remaining participant-control work now narrows to capability-lifecycle controls (delegation
issuance, revocation, and succession handoff); the roster (`admit`/`remove_member`) slice is closed
by Plan 137.
