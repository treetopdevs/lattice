...
5. **G1 (physical BEAM carrier) is now reachable outside `Sim`** — plan 017 runs W0–W3
  ...
  carrier-frame fallback when the split evidence store is empty, run a Vue sync action
   ...
   then prove Android release browser-backed onboarding state exchange where the runtime state-bearing browser link drives the same onboarding pull-author-reload-push-report flow, and prove Android release browser-backed onboarding child-grant runtime state exchange where that same app-minted runtime state gate drives the child-grant pull-grant-author-reload-push-report flow.
   Full mobile onboarding, iOS cold-start URL delivery, chooser UX,
   chooser-backed or cross-device pairing state
