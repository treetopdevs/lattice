...
5. **G1 (physical BEAM carrier) is now reachable outside `Sim`** — plan 017 runs W0–W3
  ...
  carrier-frame fallback when the split evidence store is empty, run a Vue sync action
   ...
   constant, and add a named Android release convergence gate that rebuilds each release probe APK
   before running its sync/reload, author/grant, and armed-pairing smokes, and prove Android release
   cold-start pairing delivery after `force-stop`/assert-not-running.
