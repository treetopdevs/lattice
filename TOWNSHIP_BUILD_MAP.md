...
- **D2.** Implement the TS `codec.ts` CBOR encoder; enable client-side op authoring + local verify.
  ...
  **Status:** partial. `codec.ts` can reproduce BEAM canonical bytes and op ids from carrier
  ...
  iOS cold-start URL delivery and visible chooser
  UI remain unproven. The remaining shell gaps are iOS
  simulator key-reuse proof, Android release root/authority origination, QR camera onboarding,
