...
3. **The TS conformance vectors are now Sim-generated for Phase B1/B2** (`township_join_w0`,
   ...
   or a quorum. Plan 144 will pin that limitation adversarially; no v7/user-facing succession
   authoring may start before a separate provenance design is reviewed.
...
`29358809212` closes Plans 141-143. Plan 139's reviewed v6 revocation implementation is locally
green, including full v1-v6 packaged convergence and Claude's final no-P0-P2 review, but its
required hosted flagship run is pending. Township succession reduction exists for honest asserted
ticks, while trustworthy dormancy provenance and any user-facing ceremony remain unimplemented;
Plan 144 is start-gated behind Plan 139's hosted result and will first make that limitation
executable. Production deployment and receipt-free W4 are also not supplied by the current work.
At the milestone level, hosted revocation evidence, trustworthy succession, production deployment,
and receipt-free W4 remain.
...
- **E1.** **Tauri v2 shell** (recommended spine): Vue 3.5 frontend + Rust core (key custody, CBOR,
  ...
  **Status:** started and deep — desktop and Android-release convergence, onboarding,
  ...
  Both are `DONE`; full local convergence and hosted run `29358809212` are green. Plan 139 locally
  adds the v6 issuer revocation ceremony, authenticated BEAM-authority feed overlay, isolated
  recipient proof, and mandatory no-build packaged gate; its hosted run remains pending. See
...
- **G1.** Build the production UI: Phoenix LiveView 1.1 (state/feeds) + Vue 3.5 islands (canvases),
  ...
  **Status:** Phase G's audit surface is implemented by Plan 121, Plan 122 adds the shared read
  ...
  roster, and delegation-grant actions. Plan 139 locally adds strict v6 revocation review/sign/sync
  and a Sim-equal recipient proof; TypeScript consumes the authenticated carrier BEAM authority
  report over independently verified frames rather than independently deciding delegation
  revocation. Its hosted gate, trustworthy succession tick provenance and user ceremony,
  production deployment, and the receipt-free W4 blocker remain. Plan 144 is a start-gated
  adversarial boundary plan, not a v7 action handoff.
