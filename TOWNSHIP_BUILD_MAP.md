...
| `clients/lattice-client/test/conformance.ts` + `test/vectors/*.json` | The harness that pins the TS reducer to Sim. | **Real**; W0, W1/W2 + perspectives, W3, five seeded randomized vectors, and the adversarial `township_succession_unproven_tick` characterization are generated by `lattice.export_vectors`. The latter pins accepted author-asserted tick behavior, not trustworthy dormancy. |
...
| `apps/lattice_core/lib/mix/tasks/lattice.export_vectors.ex` | Elixir task that emits conformance vectors *from Sim* — makes the oracle literal. | **Real** for Phase B1/B2 and C3a; emits fixed, randomized, and carrier W1 vectors with grant-quarantine, revocation lifecycle, and unproven-succession-tick fixtures. |
