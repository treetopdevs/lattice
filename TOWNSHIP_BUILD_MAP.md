...
5. **G1 (physical BEAM carrier) is now reachable outside `Sim`** — plan 017 runs W0–W3
  ...
  carrier-frame fallback when the split evidence store is empty, run a Vue sync action
   ...
   then prove Android release browser-backed onboarding state exchange where the runtime state-bearing browser link drives the same onboarding pull-author-reload-push-report flow.
   Full mobile onboarding, iOS cold-start URL delivery, chooser UX,
   browser-backed child-grant runtime state exchange, chooser-backed or cross-device pairing state
   exchange, authority origination, QR camera onboarding, LAN discovery, and physical-device behavior remain unproven;
...
- **D2.** Implement the TS `codec.ts` CBOR encoder; enable client-side op authoring + local verify.
  ...
  **Status:** partial. `codec.ts` can reproduce BEAM canonical bytes and op ids from carrier
  ...
  does not prove chooser UI, cross-device exchange, authority origination, browser-backed child-grant
  runtime state exchange, or full mobile onboarding.
