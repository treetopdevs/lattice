...
4. **`township_demo.exs` and the overlay now run against the branch.** Plan 121 adds the
   ...
   **unsigned request** containing only public text, target replica, and a diagnostic intent id. The
   ...
   Plan 135 adds the versioned clerk status action handoff (in progress; hosted acceptance pending).
   ...
   Hosted acceptance remains pending, and Plan 135 remains in progress.
...
Plan 135 is the active first clerk-status slice toward **complete G1/Phase G**. Broader participant
controls beyond close/reopen, production deployment, and receipt-free W4 are not supplied by the
current work. The remaining frontier still includes broader participant controls, production deployment, and receipt-free W4.
